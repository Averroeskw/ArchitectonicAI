# ArchieVerse Windows Platform Startup Flow

## Overview

ArchieVerse on Windows operates through a hybrid architecture combining native Windows services with WSL2 (Windows Subsystem for Linux) for Docker containerization. The Windows version supports both traditional Win32 API integration and modern Windows features.
...
graph TB
    subgraph "Windows System Environment"
        subgraph "Hardware Layer"
            ...
            STORAGE[Storage: C:\Users\{User}\AppData\Roaming\archie-verse\]
        end
...
sequenceDiagram
    ...
    Electron->>Windows: Register Protocol Handler (archie://)
...
graph TB
    subgraph "Windows Host"
        ARCHIE_APP[ArchieVerse App<br/>Native Windows]
...
graph TD
    ...
    subgraph "Windows System Resources"
        ...
        MODELS[Model Storage<br/>%APPDATA%\archie-verse\]
...
```
HKEY_CURRENT_USER\Software\archie-ai\archie-verse\
├── InstallPath          : REG_SZ : C:\Users\{User}\AppData\Local\Programs\archie-verse\
├── DataPath            : REG_SZ : C:\Users\{User}\AppData\Roaming\archie-verse\
...
set ARCHIE_DATA_DIR=%APPDATA%\archie-verse
...
```
%APPDATA%\archie-verse\                    # Main application data
...
%APPDATA%\archie-verse\logs\             # Application logs
...
%LOCALAPPDATA%\Programs\archie-verse\    # Application installation
├── archie-verse.exe                    # Main executable
...
```powershell
# ArchieVerse PowerShell Management Module

# Check system requirements
function Test-ArchieVerseRequirements {
    $requirements = @{
...
# Manage ArchieVerse services
function Start-ArchieVerseServices {
    Start-Service -Name "archie-llamaswap" -ErrorAction SilentlyContinue
    Start-Service -Name "archie-watchdog" -ErrorAction SilentlyContinue
    Write-Host "ArchieVerse services started"
}

function Stop-ArchieVerseServices {
    Stop-Service -Name "archie-llamaswap" -Force -ErrorAction SilentlyContinue
    Stop-Service -Name "archie-watchdog" -Force -ErrorAction SilentlyContinue
    Write-Host "ArchieVerse services stopped"
}
```
...
```powershell
# Windows Firewall Rules for ArchieVerse
New-NetFirewallRule -DisplayName "ArchieVerse - Python Backend" -Direction Inbound -Port 5001 -Protocol TCP -Action Allow
New-NetFirewallRule -DisplayName "ArchieVerse - N8N Automation" -Direction Inbound -Port 5678 -Protocol TCP -Action Allow
New-NetFirewallRule -DisplayName "ArchieVerse - ComfyUI Studio" -Direction Inbound -Port 8188 -Protocol TCP -Action Allow
New-NetFirewallRule -DisplayName "ArchieVerse - LlamaSwap Main" -Direction Inbound -Port 8091 -Protocol TCP -Action Allow
New-NetFirewallRule -DisplayName "ArchieVerse - LlamaSwap Proxy" -Direction Inbound -Port 9999 -Protocol TCP -Action Allow

# Allow ArchieVerse application through firewall
New-NetFirewallRule -DisplayName "ArchieVerse Application" -Direction Inbound -Program "C:\Users\{User}\AppData\Local\Programs\archie-verse\archie-verse.exe" -Action Allow
```
...
| Python Backend | 5000 | 5001 | HTTP | ArchieVerse - Python Backend |
| N8N Automation | 5678 | 5678 | HTTP | ArchieVerse - N8N Automation |
| ComfyUI Studio | 8188 | 8188 | HTTP/WS | ArchieVerse - ComfyUI Studio |
| LlamaSwap Main | N/A | 8091 | HTTP | ArchieVerse - LlamaSwap Main |
| LlamaSwap Proxy | 9999 | 9999 | HTTP | ArchieVerse - LlamaSwap Proxy |
...
6. **Windows Defender/Antivirus Issues**
   ```powershell
   # Add ArchieVerse to Windows Defender exclusions
   Add-MpPreference -ExclusionPath "C:\Users\$env:USERNAME\AppData\Local\Programs\archie-verse\"
   Add-MpPreference -ExclusionPath "C:\Users\$env:USERNAME\AppData\Roaming\archie-verse\"
   Add-MpPreference -ExclusionProcess "archie-verse.exe"
...
- Application logs: `%APPDATA%\archie-verse\logs\`
...
```powershell
# Monitor ArchieVerse performance
Get-Counter -Counter "\Process(archie-verse)\% Processor Time"
Get-Counter -Counter "\Process(archie-verse)\Working Set"
...
This comprehensive documentation covers the complete Windows startup flow, WSL2 integration, GPU acceleration, and Windows-specific optimizations for ArchieVerse. 