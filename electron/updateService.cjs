...
class PlatformUpdateService {
  constructor() {
    ...
    this.githubRepo = 'badboysm890/ArchieVerse';
    ...
    try {
      ...
      if (!hasUpdate) {
        return await dialog.showMessageBox({
          ...
          message: 'You are running the latest version of Archie.',
      ...
      if (this.isOTASupported()) {
        ...
        return await dialog.showMessageBox({
          ...
          message: `Archie ${latestVersion} is available`,
...
class LlamacppUpdateService {
  ...
  async downloadAndInstallBinaries(updateInfo) {
    ...
    try {
      ...
      const tempDir = path.join(require('os').tmpdir(), 'archie-llamacpp-update');
      ...
      try {
        ...
        // Backup current official llama.cpp files only (preserving Archie's custom files)
        ...
        if (fsSync.existsSync(targetPlatformDir)) {
          ...
          // Only backup official llama.cpp files, leave Archie's custom files alone
          const files = await fs.readdir(targetPlatformDir);
          for (const file of files) {
            // Only backup official llama.cpp files (not Archie's custom binaries)
        ...
        logger.info(`Archie's custom binaries (like llama-swap) were preserved and not modified`);
        if (cudaRuntimeFiles.length > 0) {
      ...
      const successMessage = `Successfully updated llama.cpp binaries and libraries to version ${updateInfo.latestVersion}. Archie's custom binaries were preserved. Services have been restarted.`;
  ...
  // Now includes ALL files except Archie's custom binaries
  isOfficialLlamacppFile(fileName) {
    // Exclude Archie's custom binaries - these are the ONLY files we don't want to touch
    const archieCustomBinaries = [
      'llama-swap',
      'llama-swap.exe',
      'llama-swap-darwin',
    ...
    // Don't touch Archie's custom binaries (llamaswap files)
    if (archieCustomBinaries.some(custom => fileName.includes(custom))) {
      return false;
    }
    ...
    async function searchDirectory(dir) {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          // Skip Archie's custom llamaswap binaries
          const archieCustomBinaries = [
            'llama-swap',
            'llama-swap.exe',
            'llama-swap-darwin',
          ...
          if (archieCustomBinaries.some(custom => entry.name.includes(custom))) {
            logger.info(`Skipping Archie custom binary: ${entry.name}`);
            continue;
          }
