...
class ComfyUIModelService extends EventEmitter {
  constructor() {
    ...
    this.containerName = 'archie_comfyui';
    
    // Local download and persistent storage directories
    const os = require('os');
    this.localDownloadDir = path.join(os.homedir(), '.archie', 'model-downloads');
    this.persistentModelDir = path.join(os.homedir(), '.archie', 'comfyui-data', 'models');
