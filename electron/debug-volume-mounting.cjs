...
class VolumeDebugger {
  ...
  async debugVolumeMounting() {
    console.log('🔍 Debugging Archie Python Backend Volume Mounting\n');
  ...
  async checkHostDirectories() {
    ...
    const directories = [
      ...
      path.join(this.pythonBackendDataPath, '.archie'),
      path.join(this.pythonBackendDataPath, '.archie', 'lightrag_storage'),
      path.join(this.pythonBackendDataPath, '.archie', 'lightrag_storage', 'metadata'),
    ];

    const files = [
      path.join(this.pythonBackendDataPath, '.archie', 'lightrag_storage', 'metadata', 'notebooks.json'),
      path.join(this.pythonBackendDataPath, '.archie', 'lightrag_storage', 'metadata', 'documents.json'),
    ];
  ...
  async checkContainerConfiguration() {
    ...
    try {
      const container = this.docker.getContainer('archie_python');
  ...
  async checkContainerVolumeMounts() {
    ...
    try {
      ...
      const archieMounts = containerInfo.Mounts.filter(mount => 
        mount.Destination === '/home/archie' || 
        mount.Source.includes('python_backend_data')
      );

      if (archieMounts.length === 0) {
        console.log('  ❌ No Archie home directory mount found!');
        console.log('  🚨 ISSUE DETECTED: python_backend_data is not mounted to /home/archie');
      } else {
        archieMounts.forEach(mount => {
          console.log(`  ✅ Found Archie mount:`);
          console.log(`      Source: ${mount.Source}`);
          console.log(`      Destination: ${mount.Destination}`);
  ...
  async testContainerFileAccess() {
    ...
    try {
      ...
      const commands = [
        'ls -la /home/archie',
        'ls -la /home/archie/.archie',
        'ls -la /home/archie/.archie/lightrag_storage',
        'ls -la /home/archie/.archie/lightrag_storage/metadata',
        'cat /home/archie/.archie/lightrag_storage/metadata/notebooks.json',
        'cat /home/archie/.archie/lightrag_storage/metadata/documents.json',
        'whoami',
        'id',
        'pwd'
      ];
  ...
  async fixVolumeMounting() {
    ...
    try {
      ...
      console.log('    ℹ️  Please restart Archie to recreate the container with proper volumes');
...
async function main() {
  ...
  if (args.includes('--fix')) {
    ...
    console.log('3. If permissions are wrong, check that Archie has read/write access to ~/.archie/');
    console.log('4. Make sure Docker has permission to access the ~/.archie directory');
  }
}
