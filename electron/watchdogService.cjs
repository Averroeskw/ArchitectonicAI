...
class WatchdogService extends EventEmitter {
  constructor(dockerSetup, llamaSwapService, mcpService, ipcLogger = null) {
    ...
    this.services.archiesCore = {
      ...
      isRetrying: false,
      enabled: true,
      healthCheck: () => this.checkArchiesCoreHealth(),
      restart: () => this.restartArchiesCore()
    };
  ...
  async checkArchiesCoreHealth() {
    try {
      ...
      this.logEvent('SERVICE_HEALTH_CHECK', 'DEBUG', 'Archie\'s Core health check completed', {
      ...
      this.logEvent('SERVICE_HEALTH_CHECK_ERROR', 'ERROR', 'Archie\'s Core health check failed', {
        error: error.message,
        stack: error.stack
      });
      return false;
  ...
  async restartArchiesCore() {
    this.logEvent('SERVICE_RESTART', 'INFO', 'Initiating Archie\'s Core service restart');
    ...
    try {
      ...
      this.logEvent('SERVICE_RESTART_OPERATION', 'INFO', 'Archie\'s Core restart operation completed');
      
    } catch (error) {
      this.logEvent('SERVICE_RESTART_ERROR', 'ERROR', 'Archie\'s Core restart operation failed', {
        error: error.message,
        stack: error.stack
      });
      throw error;
