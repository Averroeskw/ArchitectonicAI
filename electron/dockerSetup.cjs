...
class DockerSetup extends EventEmitter {
  constructor() {
    ...
    this.appDataPath = path.join(os.homedir(), '.archie');
    ...
    this.containers = {
      python: {
        ...
        volumes: [
          // Mount the python_backend_data folder as the archie user's home directory
          `${this.pythonBackendDataPath}:/home/archie`,
          // Keep backward compatibility for existing data paths
          'archie_python_models:/app/models'
        ],
        volumeNames: ['archie_python_models']
      },
      n8n: {
        name: 'archie_n8n',
      ...
      comfyui: {
        name: 'archie_comfyui',
        image: this.getArchSpecificImage('archie17verse/archie-comfyui', 'with-custom-nodes'),
        port: 8188,
        internalPort: 8188,
    ...
    // Archie container names
    this.containerNames = ['archie_python', 'archie_n8n', 'archie_comfyui'];
  ...
  initializePythonBackendDirectories() {
    try {
      ...
      const pythonDirectories = [
        '.archie',                    // Archie configuration directory
        '.archie/lightrag_storage',   // RAG storage directory
        '.archie/lightrag_storage/metadata', // Metadata for notebooks and documents
      ...
      const metadataFiles = [
        { file: '.archie/lightrag_storage/metadata/notebooks.json', content: '{}' },
        { file: '.archie/lightrag_storage/metadata/documents.json', content: '{}' }
      ];
  ...
  getPythonBackendInfo() {
    return {
      ...
      mountPoint: '/home/archie',
      description: 'All Python backend data is stored in a dedicated folder and mounted as the archie user home directory',
      structure: {
        '.archie/': 'Archie configuration and storage',
        '.archie/lightrag_storage/': 'RAG system data and embeddings',
        '.archie/lightrag_storage/metadata/': 'Notebook and document metadata',
  ...
  async optimizeComfyUIContainer() {
    try {
      ...
      const container = this.docker.getContainer('archie_comfyui');
  ...
  getComfyUIVolumes() {
    ...
    const comfyUIDataDir = path.join(os.homedir(), '.archie', 'comfyui-data');
  ...
  getArchSpecificImage(baseImage, tag) {
    // Special handling for archie-backend images which have architecture-specific tags
    if (baseImage === 'archie17verse/archie-backend') {
      const arch = os.arch();
      const platform = os.platform();
      
      console.log(`Getting archie-backend image for platform: ${platform}, arch: ${arch}`);
        ...
        if (err) {
          ...
          this.docker.pull(imageName, {}, (fallbackErr, fallbackStream) => {
            if (fallbackErr) {
              ...
              // Special fallback for archie-backend images: try base image without -amd64 suffix
              if (imageName.includes('archie17verse/archie-backend') && imageName.includes('-amd64')) {
                const baseImageName = imageName.replace('-amd64', '');
                console.log(`Trying base archie-backend image: ${baseImageName}`);
                statusCallback(`Trying base archie-backend image: ${baseImageName}`);
                
                this.docker.pull(baseImageName, {}, (baseErr, baseStream) => {
                  if (baseErr) {
                    console.error('Error pulling base archie-backend image:', baseErr);
                    reject(baseErr);
                    return;
                  }
  ...
  handlePullStream(stream, imageName, statusCallback, resolve, reject) {
    ...
    if (isFirstTimePull) {
      ...
      statusCallback(`This may take 5-15 minutes depending on your internet speed. Archie is downloading essential AI components - please wait...`, 'info', { percentage: 5 });
    }
  ...
  async createNetwork() {
    try {
      ...
      const networkExists = networks.some(network => network.Name === 'archie_network');
      
      if (networkExists) {
        console.log('Network archie_network already exists, skipping creation');
        return;
      }
      ...
      try {
        ...
        console.log('Successfully created archie_network');
      } catch (error) {
      ...
      for (const volumeInfo of volumesToCreate) {
        try {
          await this.docker.createVolume({
            ...
            Driver: 'local',
            Labels: {
              'archie.service': volumeInfo.service,
              'archie.managed': 'true'
            }
          });
  ...
  async startContainer(config) {
    try {
      ...
      const containerConfig = {
        ...
        Env: [
          'PYTHONUNBUFFERED=1',
          'OLLAMA_BASE_URL=http://archie_ollama:11434',
      ...
      if (config.name === 'archie_comfyui' && useGPURuntime) {
  ...
  async setup(selectedFeatures, statusCallback) {
    ...
    try {
      ...
      if (error.message.includes('timed out')) {
        ...
        statusCallback('💡 Try restarting Docker Desktop and Archie, or check your internet connection.', 'info');
  ...
  async performSetup(selectedFeatures, statusCallback) {
    try {
      ...
      if (isFirstTimeSetup) {
        statusCallback(`🎉 Welcome to Archie! Setting up your AI environment for the first time...`, 'info', { percentage: 5 });
  ...
  async stop() {
    try {
      ...
      // Clean up network
      try {
        const network = await this.docker.getNetwork('archie_network');
  ...
  async cleanupDockerVolumes() {
    try {
      console.log('Cleaning up Archie-managed Docker volumes...');
      ...
      // Find Archie-managed volumes
      const archieVolumes = volumes.Volumes.filter(vol => 
        vol.Labels && vol.Labels['archie.managed'] === 'true'
      );
      
      // Remove Archie-managed volumes
      for (const volume of archieVolumes) {
        try {
      ...
      if (archieVolumes.length === 0) {
        console.log('No Archie-managed volumes found to clean up');
      }
  ...
  async resolveImageName(baseImage, tag) {
    // For archie-backend, we need to test which variant is actually available
    if (baseImage === 'archie17verse/archie-backend') {
      const arch = os.arch();
