...
class PlatformManager {
  constructor(baseDir) {
    ...
    this.systemResourcesConfigPath = path.join(app.getPath('userData'), 'archie-system-config.yaml');
  ...
  getUnsupportedPlatformInstructions(platform) {
    return {
      ...
      description: `The platform ${platform} is not currently supported by ArchieVerse.`,
      minimumRequired: 'Supported platforms: Windows, macOS, Linux',
      instructions: [
        'ArchieVerse currently supports:',
        '• Windows 10+ (x64)',
        '• macOS Big Sur 11.0+ (Intel and Apple Silicon)',
        '• Linux x64 with kernel 4.4+ (Ubuntu, Debian, CentOS, RHEL, Fedora, etc.)',
        '',
        'Please use one of these supported platforms to run ArchieVerse.'
      ],
      downloadLinks: [
