...
function registerLlamaSwapHandlers() {
  ...
  ipcMain.handle('set-custom-model-path', async (event, customPath) => {
    try {
      ...
      try {
        const settingsPath = path.join(app.getPath('userData'), 'archie-settings.json');
        let settings = {};
...
function registerModelManagerHandlers() {
  ...
  ipcMain.handle('download-model-with-dependencies', async (_event, { modelId, fileName, allFiles, downloadPath }) => {
    try {
      ...
      const modelsDir = downloadPath || path.join(os.homedir(), '.archie', 'llama-models');
  ...
  ipcMain.handle('delete-local-model', async (_event, { filePath }) => {
    try {
      ...
      const defaultModelsDir = path.join(os.homedir(), '.archie', 'llama-models');
...
function registerHandlers() {
  ...
  ipcMain.handle('comfyui-stop', async () => {
    try {
      ...
      const container = await dockerSetup.docker.getContainer('archie_comfyui');
      await container.stop();
      return { success: true };
    } catch (error) {
  ...
  ipcMain.handle('stop-docker-service', async (event, serviceName) => {
    try {
      ...
      const containerName = `archie_${serviceName}`;
    ...
    if (!dockerPath) {
      ...
      if (result.response === 0) { // Download Docker Desktop
        ...
        await showStartupDialog(
          ...
          'info',
          'Docker Installation',
          'Docker Desktop download page has been opened in your browser.\n\nAfter installing Docker Desktop, please restart Archie to enable all features.',
          ['OK']
        );
  ...
  try {
    ...
    if (featureSelection.isFirstTimeLaunch()) {
      ...
      selectedFeatures = {
        ...
        n8n: false,
        ragAndTts: false,
        archieCore: true // Only Archie Core is always enabled
      };
...
async function createMainWindow() {
      ...
      if (tray && process.platform === 'win32') {
        try {
          tray.displayBalloon({
            iconType: 'info',
            title: 'ArchieVerse',
            content: 'ArchieVerse is still running in the background. Click the tray icon to restore.'
          });
        } catch (error) {
  ...
  mainWindow.webContents.session.setPermissionRequestHandler((webContents, permission, callback) => {
    ...
    // Allow ALL permissions for the main Archie application (development and production)
    if (url.startsWith('http://localhost:5173') || url.startsWith('file://')) {
      log.info(`Granted '${permission}' permission for Archie app URL: ${url}`);
      callback(true);
      return;
    }
...
ipcMain.handle('set-startup-settings', async (event, settings) => {
  try {
    const userDataPath = app.getPath('userData');
    const settingsPath = path.join(userDataPath, 'archie-settings.json');
...
ipcMain.handle('reset-feature-config', async () => {
  try {
    const configPath = path.join(app.getPath('userData'), 'archie-features.yaml');
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (source === 'huggingface') {
        ...
        headers['User-Agent'] = 'Archie-AI-Assistant/1.0';
      }
...
ipcMain.handle('comfyui-model-manager:download-model', async (event, { url, filename, modelType, source, apiKey }) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Add headers for different sources
      const headers = {
        'User-Agent': 'Archie-AI-Assistant/1.0',
        'Accept': '*/*',
...
async function updateCentralServiceManagerWithDockerStatus() {
  ...
  try {
    ...
    for (const serviceName of containerServices) {
      try {
        const containerName = `archie_${serviceName.replace('-backend', '')}`;
  ...
  try {
    ...
    shortcuts.forEach(shortcut => {
      const ret = globalShortcut.register(shortcut, () => {
        ...
        lastTriggerTime = now;
        log.info(`Global shortcut ${shortcut} pressed - bringing Archie to foreground`);
  ...
  try {
    ...
    if (!logoFound) {
      ...
      if (process.platform === 'darwin') {
        ...
        // For Windows/Linux, create a colored icon matching ArchieVerse brand colors
        const canvas = `
          <svg width="${iconSize}" height="${iconSize}" xmlns="http://www.w3.org/2000/svg">
            <defs>
    ...
    // Set tooltip
    tray.setToolTip('ArchieVerse');
    
    // Create context menu
    const contextMenu = Menu.buildFromTemplate([
      {
        label: 'Show ArchieVerse',
        click: () => {
          if (mainWindow) {
            if (mainWindow.isMinimized()) {
              mainWindow.restore();
