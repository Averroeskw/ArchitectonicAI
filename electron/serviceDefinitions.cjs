/**
 * ArchieVerse Service Definitions
...
// Base paths
const appDataPath = path.join(os.homedir(), '.archie');
...
const SERVICE_DEFINITIONS = {
  ...
  'python-backend': {
    ...
    dockerContainer: {
      name: 'archie_python',
      image: 'archie17verse/archie-backend:latest',
      ports: { '5001': '5000' },
      volumes: [
        `${pythonBackendDataPath}:/home/archie`,
  ...
  // LlamaSwap Service (Archie's AI model manager)
  llamaswap: {
    name: "Archie's Core AI Engine",
    type: 'binary',
    critical: true,
    autoRestart: true,
    priority: 3,
...
function getEnabledServices(selectedFeatures = {}) {
  ...
  // Archie Core AI (always enabled if selected)
  if (selectedFeatures.archieCore !== false) {
