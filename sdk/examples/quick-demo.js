...
/**
 * Archie Flow SDK - Quick Demo
 * 
 * This script demonstrates the key features of the Archie Flow SDK:
...
import { ArchieFlowRunner } from '../dist/archie-flow-sdk.esm.js';

console.log('🚀 Archie Flow SDK - Quick Demo\n');
  ...
  nodes: [
    {
      ...
      type: 'input',
      name: 'User Message',
      data: { inputType: 'string', defaultValue: 'Hello Archie!' },
      inputs: [],
...
try {
  const result1 = await runner.executeFlow(simpleFlow, {
    'User Message': 'Hello from the Archie Flow SDK!'
  });
...
try {
  ...
  const result2 = await runner.executeFlow(agentStudioExport, {
    'Question': 'How easy is it to use Archie Flow SDK?'
  });
...
try {
  const result3 = await runner.executeFlow(customNodeFlow, {
    'Original Text': 'The Archie Flow SDK is amazing'
  });
  ...
  nodes: [
    {
      ...
      type: 'input',
      name: 'Your Name',
      data: { inputType: 'string', defaultValue: 'Archie User' },
      inputs: [],
      outputs: [{ id: 'output', name: 'Value', type: 'output', dataType: 'string' }]
    },
    {
...
console.log('🚀 Ready to build amazing AI workflows with Archie Flow SDK!');
