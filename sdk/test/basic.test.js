/**
 * Basic test for Archie Flow SDK v2.0
 */

import { ArchieFlowRunner, BrowserUtils } from '../src/index.js';

console.log('🧪 Testing Archie Flow SDK v2.0...\n');
    ...
    nodes: [
      {
        ...
        type: 'input',
        name: 'User Input',
        data: { value: 'Hello from Archie SDK!' },
        inputs: [],
...
console.log('\nTest 3: JSON Parsing Workflow');
try {
  const runner = new ArchieFlowRunner({ enableLogging: false });
...
// Test 6: Archie Studio export format compatibility
console.log('\nTest 6: Archie Studio Export Format');
try {
  const runner = new ArchieFlowRunner({ enableLogging: false });
  
  // Simulate Archie Studio export format
  const studioExport = {
    format: 'archie-sdk',
    version: '1.0.0',
    flow: {
      ...
      nodes: [
        {
          ...
          type: 'input',
          name: 'Test Input',
          data: { value: 'Archie Studio Export Test' },
          inputs: [],
          outputs: [{ id: 'output', name: 'Output', dataType: 'string' }]
        },
        {
  ...
  console.log('✅ Archie Studio export format handled successfully');
  console.log('📤 Result:', result['output-1']?.output);
  
} catch (error) {
  console.error('❌ Archie Studio export format failed:', error.message);
  process.exit(1);
}

console.log('\n🎉 All tests passed! Archie Flow SDK v2.0 is working correctly.');
...
console.log('✅ Archie Studio export format compatibility');
