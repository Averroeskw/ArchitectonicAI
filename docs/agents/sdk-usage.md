...
Learn how to integrate Archie Flow SDK v2.0 into your applications. Execute AI workflows with just a few lines of code, with automatic input detection and error handling.
...
### **ArchieFlowRunner Class**

**Constructor:**
```javascript
const runner = new ArchieFlowRunner(options);
```
...
**Parameters:**
- `workflow` (Object): Exported workflow from Archie Studio
...
export function createRunner(environment = 'development') {
  ...
  return new ArchieFlowRunner(configs[environment]);
}
...
function WorkflowExecutor({ workflow }) {
  const [runner] = useState(() => new ArchieFlowRunner());
...
class OptimizedRunner extends ArchieFlowRunner {
  constructor(options = {}) {
    super({
      ...options,
...
4. **"Invalid workflow format"** - Check workflow was exported correctly from Archie Studio
...
**Ready to integrate Archie Flow SDK?** Start with the basic examples and gradually add advanced features! 🚀 