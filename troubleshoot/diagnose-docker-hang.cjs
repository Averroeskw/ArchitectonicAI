...
/**
 * Archie Docker Startup Diagnostic Tool
 * 
 * This script helps diagnose and resolve Docker startup hang issues.
 * Run this when Archie gets stuck after "Update check complete" message.
 */
...
class DockerDiagnostic {
  ...
  async run() {
    console.log('🔍 Archie Docker Startup Diagnostic Tool');
    console.log('=====================================\n');
  ...
  async checkDockerImages() {
    console.log('\n📦 Checking Archie Docker Images...');
    
    const archieImages = [
      'archie17verse/archie-backend',
      'archie17verse/archie-comfyui',
      'n8nio/n8n'
    ];

    for (const imageName of archieImages) {
      try {
  ...
  async checkRunningContainers() {
    console.log('\n🔄 Checking Running Archie Containers...');
    
    const archieContainers = ['archie_python', 'archie_comfyui', 'archie_n8n'];
    
    try {
      const { stdout } = await execAsync('docker ps --format "{{.Names}} {{.Status}} {{.Ports}}"');
      const runningContainers = stdout.trim().split('\n').filter(line => line.trim());
      
      for (const containerName of archieContainers) {
  ...
  async checkDockerLogs() {
    console.log('\n📋 Checking Archie Container Logs...');
  ...
  provideSolutions() {
    ...
    console.log('   3. Restart Archie application');
    console.log('   4. If still stuck, check Archie logs in console');
    console.log('   5. Try running Archie with: DEBUG=1 npm start');
    
    console.log('\n🔧 Emergency Recovery Commands:');
    console.log('   # Stop all Archie containers');
    console.log('   docker stop archie_python archie_comfyui archie_n8n 2>/dev/null || true');
    console.log('   ');
    console.log('   # Remove all Archie containers');
    console.log('   docker rm -f archie_python archie_comfyui archie_n8n 2>/dev/null || true');
    console.log('   ');
    console.log('   # Clean up Archie network');
    console.log('   docker network rm archie_network 2>/dev/null || true');
    console.log('   ');
