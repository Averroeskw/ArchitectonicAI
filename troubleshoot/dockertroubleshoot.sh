...
# Archie Docker Troubleshooting & Fix Script for macOS
# This script diagnoses and fixes common Docker detection issues for Archie
...
check_docker_sockets() {
    ...
    # Archie's socket search order (from dockerSetup.cjs)
    SOCKET_PATHS=(
...
# Test Archie-specific Docker operations
test_archie_operations() {
    print_header "TESTING ARCHIE-SPECIFIC DOCKER OPERATIONS"
    
    # Test network creation (Archie creates archie_network)
    print_status "Testing Docker network operations..."
    
    # Check if archie_network exists
    if docker network ls --format "{{.Name}}" | grep -q "^archie_network$"; then
        print_success "Archie network already exists"
    else
        print_status "Creating Archie network..."
        if docker network create archie_network >/dev/null 2>&1; then
            print_success "Created Archie network successfully"
            
            # Clean up test network
            docker network rm archie_network >/dev/null 2>&1
            print_status "Cleaned up test network"
        else
            print_error "Failed to create Archie network"
            return 1
        fi
    fi
...
# Check Archie's specific ports
check_archie_ports() {
    print_header "CHECKING ARCHIE SERVICE PORTS"
    
    ARCHIE_PORTS=(
        "5001:Archie Python Backend"
        "5678:Archie N8N"
        "11434:Ollama (if installed)"
    )
    
    for port_info in "${ARCHIE_PORTS[@]}"; do
        PORT="${port_info%%:*}"
        SERVICE="${port_info##*:}"
...
run_full_diagnosis() {
    print_header "ARCHIE DOCKER TROUBLESHOOTING DIAGNOSTIC"
    ...
    test_archie_operations || ((issues_found++))
    check_archie_ports || true  # Don't count port conflicts as failures
...
generate_recommendations() {
    ...
    if [[ $issues_count -eq 0 ]]; then
        print_success "All checks passed! Docker is working correctly for Archie."
        print_status "You can now start Archie and Docker services should work properly."
    else
        ...
        echo "4. Run this script again: ./archie-docker-fix.sh"
        echo ""
...
# Interactive mode
interactive_mode() {
    print_header "ARCHIE DOCKER INTERACTIVE TROUBLESHOOTER"
    
    echo "This script will help diagnose and fix Docker issues for Archie on macOS."
    echo ""
    echo "What would you like to do?"
    echo "1) Run full diagnosis"
    echo "2) Quick fix (restart Docker Desktop)"
    echo "3) Check Docker status only"
    echo "4) Test Archie operations only"
    echo "5) Exit"
    echo ""
    ...
    if [[ $# -eq 0 ]]; then
        ...
        case "$1" in
            ...
            --archie)
                ...
                ;;
            --help|-h)
                echo "Archie Docker Troubleshooting Script"
                echo ""
                ...
                echo "  --archie          Test Archie-specific operations only"
