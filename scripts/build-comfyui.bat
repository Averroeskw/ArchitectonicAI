@echo off
:: ComfyUI Docker Build Script for Archie (Windows)
setlocal enabledelayedexpansion

echo 🎨 Building Archie ComfyUI Docker Image...
...
set "IMAGE_NAME=archie17verse/archie-comfyui"
...
if %ERRORLEVEL% equ 0 (
    ...
    echo.
    echo 🚀 You can now start Archie and use the bundled ComfyUI!
...
if /i "%test_image%"=="y" (
    ...
    docker stop archie_comfyui_test >nul 2>&1
    docker rm archie_comfyui_test >nul 2>&1
    
    :: Run test container
    echo 🔄 Starting test container...
    docker run -d --name archie_comfyui_test --platform linux/%DOCKER_ARCH% -p 8188:8188 --gpus all "%FULL_IMAGE_NAME%"
    ...
    if %ERRORLEVEL% equ 0 (
        ...
        echo    docker stop archie_comfyui_test && docker rm archie_comfyui_test
    ) else (
        echo ❌ ComfyUI test failed - container may still be starting
        echo 📋 Check logs with: docker logs archie_comfyui_test
    )
)
