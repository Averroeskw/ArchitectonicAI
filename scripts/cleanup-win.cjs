...
try {
  ...
  console.log('Terminating any running instances of Archie...');
  try {
    execSync('taskkill /f /im Archie.exe', { stdio: 'ignore' });
    console.log('Successfully terminated Archie processes');
  } catch (e) {
    // It's okay if no processes were found to kill
    console.log('No running Archie processes found or could not terminate');
  }
  ...
  const installerPath = path.join(releaseDir, 'Archie-1.0.0-win.exe');
