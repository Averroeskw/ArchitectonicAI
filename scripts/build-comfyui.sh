...
# ComfyUI Docker Build Script for Archie
set -e
...
IMAGE_NAME="archie17verse/archie-comfyui"
...
if [[ "$test_image" =~ ^[Yy]$ ]]; then
    ...
    docker stop archie_comfyui_test 2>/dev/null || true
    docker rm archie_comfyui_test 2>/dev/null || true
    
    # Run test container
    echo "🔄 Starting test container..."
    docker run -d \
        --name archie_comfyui_test \
        --platform linux/$DOCKER_ARCH \
        -p 8188:8188 \
        --gpus all \
        "$FULL_IMAGE_NAME"
