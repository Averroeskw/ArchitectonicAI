const STORAGE_KEY = 'archie_provider_configs';

import type { ArchieAIConfig } from '../types/archie_assistant_types';

export function saveProviderConfig(providerId: string, config: ArchieAIConfig) {
...
export function loadProviderConfig(providerId: string): ArchieAIConfig | null {
  const all = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}');
...
export function getAllProviderConfigs(): Record<string, ArchieAIConfig> {
...
export function validateProviderConfig(providerId: string, config: ArchieAIConfig, availableModels: string[]): ArchieAIConfig {
  const cleanConfig = {
    ...config,
