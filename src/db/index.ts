...
export interface Provider {
  id: string;
  name: string;
  type: 'archies-pocket' | 'openai' | 'openai_compatible' | 'ollama' | 'openrouter';
  baseUrl?: string;
  apiKey?: string;
  isEnabled: boolean;
  isPrimary: boolean;
...
const DB_PREFIX = 'archie_db_';

const DEFAULT_SYSTEM_PROMPT = `You are Archie, a helpful and friendly AI assistant with advanced artifact generation capabilities. Your responses automatically create beautiful, interactive components that enhance user experience.
...
export class LocalStorageDB {
  ...
  async addProvider(provider: Omit<Provider, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
      // Prevent multiple Archie's Pocket providers with double-check
      if (provider.type === 'archies-pocket') {
        console.log('Checking for existing Archie\'s Pocket providers before adding...');
        const providers = await this.getAllProviders();
        const existingArchiesPocket = providers.find(p => p.type === 'archies-pocket');
        if (existingArchiesPocket) {
          console.log(`Archie's Pocket provider already exists: ${existingArchiesPocket.name} (ID: ${existingArchiesPocket.id})`);
          throw new Error("Archie's Pocket provider already exists. Only one instance is allowed.");
        }
        console.log('No existing Archie\'s Pocket provider found, proceeding with creation...');
      }
      ...
      if (provider.type === 'archies-pocket') {
        ...
        const stillNoExisting = recentProviders.find(p => p.type === 'archies-pocket');
        if (stillNoExisting) {
          console.log(`Race condition detected: Archie's Pocket provider was created during our check: ${stillNoExisting.name} (ID: ${stillNoExisting.id})`);
          throw new Error("Archie's Pocket provider was created by another process. Only one instance is allowed.");
        }
      }
  ...
  async initializeDefaultProviders(): Promise<void> {
    ...
    try {
      ...
      // Clean up any duplicate Archie's Pocket providers (keep only the first one)
      const archiesPocketProviders = providers.filter(p => p.type === 'archies-pocket');
      if (archiesPocketProviders.length > 1) {
        console.log(`Found ${archiesPocketProviders.length} duplicate Archie's Pocket providers, cleaning up...`);
        // Keep the first one, delete the rest
        for (let i = 1; i < archiesPocketProviders.length; i++) {
          console.log(`Deleting duplicate Archie's Pocket provider: ${archiesPocketProviders[i].name} (ID: ${archiesPocketProviders[i].id})`);
          await this.deleteProvider(archiesPocketProviders[i].id);
        }
      ...
      // Check if Archie's Core exists
      const archiesCoreExists = updatedProviders.some(p => p.type === 'archies-pocket');
      
      // Create Archie's Core if it doesn't exist - this should be the primary provider
      if (!archiesCoreExists) {
        console.log('No Archie\'s Core found, creating one...');
        ...
        console.log('Archie\'s Core created successfully');
      } else {
        console.log('Archie\'s Core already exists, skipping creation');
      }
        ...
        try {
          ...
          if (response.ok) {
            ...
            await this.addProvider({
              ...
              isEnabled: true,
              isPrimary: false, // Archie's Core should remain primary
              config: {
      ...
      // Ensure at least one provider is primary (should be Archie's Core)
      const finalProviders = await this.getAllProviders();
      const primaryProvider = finalProviders.find(p => p.isPrimary);
      if (!primaryProvider) {
        console.log('No primary provider found, setting Archie\'s Core as primary...');
        const archiesCoreProvider = finalProviders.find(p => p.type === 'archies-pocket');
        if (archiesCoreProvider) {
          await this.setPrimaryProvider(archiesCoreProvider.id);
          console.log('Archie\'s Core set as primary provider');
        } else {
          // Fallback: set first enabled provider as primary
          const enabledProvider = finalProviders.find(p => p.isEnabled);
          if (enabledProvider) {
            console.log('No Archie\'s Core found, setting first enabled provider as primary...');
