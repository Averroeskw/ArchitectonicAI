...
export const AgentBuilderProvider: React.FC<AgentBuilderProviderProps> = ({ children }) => {
  ...
  const DRAFT_STATE_KEY = 'archie-verse-draft-state';
  ...
  const generateFlowCode = (flow: AgentFlow, flowNodes: FlowNode[], flowConnections: Connection[], customNodeTypes: Set<string>): string => {
    ...
    const flowData = {
      ...
      metadata: {
        ...
        exportedFrom: 'Archie Agent Studio',
 ...
 * Flow: ${flow.name}
 * Description: ${flow.description || 'AI workflow generated from Archie Agent Studio'}
...
*/`;
  ...
  const exportFlow = async (format: string = 'archie-native') => {
    if (!currentFlow) {
    ...
    try {
      ...
      if (format === 'archie-sdk') {
        ...
        // archie-native format (default) - now includes custom nodes
        exportData = {
          format: 'archie-native',
          version: '1.0.0',
          flow: {
            id: currentFlow.id,
            name: currentFlow.name,
