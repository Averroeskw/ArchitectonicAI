...
function App() {
  ...
  // Listen for global shortcut trigger to navigate to Archie chat
  useEffect(() => {
    let lastTriggerTime = 0;
    const debounceDelay = 300; // 300ms debounce
    
    const handleGlobalArchieShortcut = () => {
      const now = Date.now();
      ...
      lastTriggerTime = now;
      console.log('Global shortcut triggered - navigating to Archie chat');
      setActivePage('archie');
    };

    // Add listener for the trigger-new-chat event
    if (window.electron && window.electron.receive) {
      window.electron.receive('trigger-new-chat', handleGlobalArchieShortcut);
    }

    // Cleanup listener on unmount
    return () => {
      if (window.electron && window.electron.removeListener) {
        window.electron.removeListener('trigger-new-chat', handleGlobalArchieShortcut);
      }
    };
  }, []);
  ...
  const renderContent = () => {
    if (activePage === 'assistant') {
      return <ArchieAssistant onPageChange={setActivePage} />;
    }
    
    // Archie is now always mounted but conditionally visible
    ...
    <ProvidersProvider>
      <div className="min-h-screen bg-gradient-to-br from-white to-sakura-100 dark:from-gray-900 dark:to-sakura-100">
        {showOnboarding ? (
          ...
          <>
            {/* Always render Archie in background - visible when activePage is 'archie' */}
            <div className={activePage === 'archie' ? 'block' : 'hidden'} data-archie-container>
              <ArchieAssistant onPageChange={setActivePage} />
            </div>
            
            {/* Render other content when not on Archie page */}
            {activePage !== 'archie' && renderContent()}
          </>
        )}
      </div>
    </ProvidersProvider>
