  ...
  const handleCleanup = async () => {
    try {
      ...
      const services = [
        ...
        // TTS cleanup
        async () => {
          const archieTTSService = (window as any).archieTTSService;
          if (archieTTSService) {
        ...
        async () => {
          ...
          // Stop and remove all Archie containers
          for (const container of containers) {
            if (container.name.startsWith('archie_')) {
              try {
  ...
  return (
    <div className="glassmorphic h-16 px-6 flex items-center justify-between relative z-[10000]">
      ...
      <div className="flex items-center gap-6">
        ...
        <NotificationPanel onNavigateToArchie={() => onPageChange?.('archie')} />
        <UserProfileButton
