...
import type { ArchieProvider, ArchieModel, ArchieAIConfig } from '../types/archie_assistant_types';
...
const ToolBelt: React.FC = () => {
    ...
    const loadProvidersAndModels = async () => {
      try {
        ...
        if (enabledProviders.length > 0) {
          ...
          const loadedModels = await archieApiService.getModels(primaryProvider.id);
  ...
  const handleProviderChange = async (providerId: string) => {
    ...
    try {
      const loadedModels = await archieApiService.getModels(providerId);
      setModels(loadedModels);
  ...
  const generateToolWithAI = async () => {
    ...
    try {
      ...
      const systemPrompt = `You are an expert JavaScript developer and tool architect. Your job is to create Archie Assistant tools based on user descriptions.
...
Remember: RESPOND WITH VALID JSON ONLY!`;
      ...
      const userPrompt = `Create a Archie Assistant tool based on this description:
...
Make it useful, robust, and user-friendly. Include proper error handling and return meaningful results.`;
      ...
      // Send request to AI
      const response = await archieApiService.sendChatMessage(
        userPrompt,
        aiConfig,
        undefined,
        systemPrompt
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Manage and create custom tools for Archie Assistant
            </p>
          </div>
        </div>
          ...
          <div className="glassmorphic rounded-xl p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Describe what you want, and AI will create a custom tool for Archie
                </p>
              </div>
            </div>
