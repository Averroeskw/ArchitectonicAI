...
import ArchieSidebar from './Archie_Components/ArchieSidebar';
import ArchieAssistantInput from './Archie_Components/archie_assistant_input';
import ArchieChatWindow from './Archie_Components/archie_assistant_chat_window';
import { AdvancedOptions } from './Archie_Components/archie_assistant_input';
import Sidebar from './Sidebar';
import { db } from '../db';
import { archieDB } from '../db/archieDatabase';

// Import Archie types and API service
import { 
  ArchieMessage, 
...
import { archieApiService } from '../services/archieApiService';
...
import { archieMCPService } from '../services/archieMCPService';
import { archieMemoryService } from '../services/archieMemoryService';
...
import { archieBackgroundService } from '../services/archieBackgroundService';

// Import clear data utility
import '../utils/clearArchieData';
import { copyToClipboard } from '../utils/clipboard';

// Import the new professional status panel
import AutonomousAgentStatusPanel from './Archie_Components/AutonomousAgentStatusPanel';
import useAutonomousAgentStatus from '../hooks/useAutonomousAgentStatus';

// Import TTS service
import { archieTTSService } from '../services/archieTTSService';
 ...
 */
const getDefaultSystemPrompt = (provider: ArchieProvider, artifactConfig?: any): string => {
...
`;
  ...
  switch (provider?.type) {
    case 'ollama':
      return `You are Archie, a helpful AI assistant powered by ${providerName}. You are knowledgeable, friendly, and provide accurate information. You can help with various tasks including analysis, coding, writing, and general questions. When using tools, be thorough and explain your actions clearly.${artifactGuidance} ${toolsGuidance}`;
      
    case 'openai':
      return `You are Archie, an intelligent AI assistant powered by OpenAI. You are helpful, harmless, and honest. You excel at reasoning, analysis, creative tasks, and problem-solving. Always strive to provide accurate, well-structured responses and use available tools effectively when needed.${artifactGuidance} ${toolsGuidance}`;
      
    case 'openrouter':
      return `You are Archie, a versatile AI assistant with access to various models through OpenRouter. You adapt your communication style based on the task at hand and leverage the strengths of different AI models. Be helpful, accurate, and efficient in your responses.${artifactGuidance} ${toolsGuidance}`;
      
    case 'archies-pocket':
      return `# Archie - Privacy-First AI 🎯

You're Archie, a tech-savvy friend on user's device. Be real, helpful, chill.
...
**Remember:** Knowledge friend who wants to help. When limited, suggest agent mode for full capabilities..${artifactGuidance} ${toolsGuidance}`;
    ...
    default:
      return `You are Archie, a helpful AI assistant. You are knowledgeable, friendly, and provide accurate information. You can help with various tasks including analysis, coding, writing, and general questions. Always be helpful and respectful in your interactions.${artifactGuidance} ${toolsGuidance}`;
  }
};

// Add a hook to detect if Archie is currently visible
const useIsVisible = () => {
  const [isVisible, setIsVisible] = useState(true);
  
  useEffect(() => {
    const checkVisibility = () => {
      // Check if the Archie container is visible
      const archieContainer = document.querySelector('[data-archie-container]');
      if (archieContainer) {
        const isCurrentlyVisible = !archieContainer.classList.contains('hidden');
    ...
    const archieContainer = document.querySelector('[data-archie-container]');
    if (archieContainer) {
      observer.observe(archieContainer, { 
        attributes: true, 
...
const ArchieAssistant: React.FC<ArchieAssistantProps> = ({ onPageChange }) => {
  // Check if Archie is currently visible (for background operation)
  const isVisible = useIsVisible();
  
  // User and session state
  const [userName, setUserName] = useState<string>('');
  const [currentSession, setCurrentSession] = useState<ArchieChatSession | null>(null);
  ...
  const [sessions, setSessions] = useState<ArchieChatSession[]>([]);
  ...
  const [providers, setProviders] = useState<ArchieProvider[]>([]);
  const [models, setModels] = useState<ArchieModel[]>([]);
  ...
  const [sessionConfig, setSessionConfig] = useState<ArchieSessionConfig>({
    aiConfig: {
      models: {
        text: '',
        vision: '',
  ...
  const checkProviderHealthCached = useCallback(async (provider: ArchieProvider): Promise<boolean> => {
    const now = Date.now();
    ...
    try {
      const isHealthy = await archieApiService.testProvider(provider);
  ...
  const refreshProvidersAndServices = useCallback(async (force: boolean = false) => {
    ...
    try {
      ...
      try {
        ...
        await archieMCPService.refresh();
      ...
      const refreshedProviders = await archieApiService.getProviders();
      ...
      let allModels: ArchieModel[] = [];
      for (const provider of refreshedProviders) {
        try {
          const providerModels = await archieApiService.getModels(provider.id);
      ...
      if (currentProviderId) {
        ...
        if (currentProvider) {
          archieApiService.updateProvider(currentProvider);
      ...
      if (sessionConfig.aiConfig?.provider) {
        ...
        if (currentProvider) {
          try {
            const isHealthy = await archieApiService.testProvider(currentProvider);
            if (!isHealthy) {
  ...
  // Track Archie's visibility state for background service
  useEffect(() => {
    archieBackgroundService.setBackgroundMode(!isVisible);
  }, [isVisible]);

  // Auto-refresh when Archie becomes visible again
  useEffect(() => {
    if (isVisible && !isLoadingProviders) {
      // Trigger refresh when Archie becomes visible
      console.log('👁️ Archie became visible - checking for updates...');
  ...
  useEffect(() => {
    const loadInitialSessions = async () => {
      ...
      try {
        ...
        const recentSessions = await archieDB.getRecentArchieSessionsLight(20); // Load only 20 initially
        ...
        if (!currentSession && recentSessions.length > 0) {
          ...
          const sessionWithMessages = await archieDB.getArchieSession(mostRecent.id);
  ...
  const loadMoreSessions = useCallback(async () => {
    ...
    try {
      const moreSessions = await archieDB.getRecentArchieSessionsLight(20, sessionPage * 20);
  ...
  useEffect(() => {
    const loadProvidersAndModels = async () => {
      ...
      try {
        ...
        archieApiService.clearBlacklistedTools();
        
        // Initialize MCP service
        try {
          await archieMCPService.initialize();
        ...
        // Load providers
        const loadedProviders = await archieApiService.getProviders();
        ...
        if (primaryProvider) {
          // AUTO-START ARCHIE'S POCKET IF IT'S THE PRIMARY PROVIDER
          if (primaryProvider.type === 'archies-pocket' && window.llamaSwap) {
            try {
              console.log("🚀 Checking Archie's Core status on startup...");
              const status = await window.llamaSwap.getStatus?.();
              if (!status?.isRunning) {
                console.log("🔄 Archie's Core is not running, starting automatically...");
                addInfoNotification(
                  "Starting Archie's Core...",
                  'Archie is starting up her local AI service for you. This may take a moment.',
                  6000
                );
                ...
                if (!result.success) {
                  addErrorNotification(
                    "Failed to Start Archie's Core",
                    result.error || 'Could not start the local AI service. Please check your installation.',
                    10000
                  );
                  console.error("❌ Failed to start Archie's Core:", result.error);
                } else {
                  console.log("✅ Archie's Core started successfully");
                  addInfoNotification(
                    "Archie's Core Ready",
                ...
                console.log("✅ Archie's Core is already running");
                addInfoNotification(
                  "Archie's Core Online",
                  'Your local AI service is ready and waiting for your messages.',
                  3000
                );
              }
            } catch (err) {
              console.error("⚠️ Error checking/starting Archie's Core:", err);
              addErrorNotification(
                "Archie's Core Startup Error",
          ...
          archieApiService.updateProvider(primaryProvider);
  ...
  useEffect(() => {
    const initializeTTS = async () => {
      try {
        ...
        const isHealthy = await archieTTSService.forceHealthCheck();
  ...
  const createRefinedStreamingResponse = useCallback(async (
    ...
    try {
      ...
      const memoryContext = archieMemoryService.generateMemoryContext();
      console.log('🧠 Memory context for refinement:', memoryContext ? 'Available' : 'Empty');

      // Create refinement prompt with memory context
      const refinementPrompt = `You are Archie, reviewing and refining an AI response to make it more user-friendly and resonant.
...
"${originalUserQuestion}"`;
      ...
      const refinementResponse = await archieApiService.sendChatMessage(
        refinementPrompt,
        streamingConfig,
  ...
  // Create new session
  const createNewSession = useCallback(async (): Promise<ArchieChatSession> => {
    try {
      const session = await archieDB.createArchieSession('New Chat');
  ...
  const handleSendMessage = useCallback(async (content: string, attachments?: ArchieFileAttachment[]) => {
    ...
    const userMessage: ArchieMessage = {
      id: generateId(),
      role: 'user',
    ...
    if (!isVisible) {
      archieBackgroundService.incrementBackgroundActivity();
    }

    // Save user message to database (with display content only)
    try {
      await archieDB.addArchieMessage(currentSession.id, userMessage);
    ...
    const streamingMessage: ArchieMessage = {
    ...
    try {
      ...
      const conversationHistory: ArchieMessage[] = [];
      ...
      const systemPrompt = enforcedConfig.systemPrompt || 
                          (currentProvider ? getDefaultSystemPrompt(currentProvider, enforcedConfig.artifacts) : 'You are Archie, a helpful AI assistant.');
      ...
      const aiMessage = await archieApiService.sendChatMessage(
      ...
      if (aiMessage.content && aiMessage.content.length > 50) {
        ...
        if (autoDetectEnabled) {
          const detectionContext: DetectionContext = {
            userMessage: content,
            conversationHistory: conversationHistory.map((msg: ArchieMessage) => msg.content),
      ...
      try {
        await archieDB.addArchieMessage(currentSession.id, finalMessage);
        ...
        if (!isVisible) {
          // More prominent notification when Archie is in background
          // Use display content for notifications (without voice prefix)
          addBackgroundCompletionNotification(
            'Archie Response Ready',
            `Archie has finished responding to: "${displayContent.slice(0, 40)}${displayContent.length > 40 ? '...' : ''}"`
          );
          // Track background notification creation
          archieBackgroundService.onBackgroundNotificationCreated();
        } else {
          // Standard notification when Archie is visible
          addCompletionNotification(
            'Chat Response Complete',
            isVoiceMessage ? 'Archie has finished responding to your voice message.' : 'Archie has finished responding to your message.',
            4000
          );
        }
      } catch (error) {
      ...
      if (currentSession.title === 'New Chat') {
        ...
        try {
          await archieDB.updateArchieSession(currentSession.id, { title: newTitle });
          setSessions(prev => prev.map(s => 
      ...
      if (isAbortError) {
        ...
        try {
          ...
          setMessages(prev => {
            ...
            if (currentMessage && currentSession) {
              ...
              archieDB.addArchieMessage(currentSession.id, abortedMessage).catch(dbError => {
        ...
        if (isVisionError || (hasImages && error instanceof Error && error.message.includes('server'))) {
          ...
          const errorMessage: ArchieMessage = {
            id: streamingMessageId,
            role: 'assistant',
...
Would you like me to help with text-only responses for now?`,
        ...
        try {
          ...
          setMessages(prev => {
            const currentMessage = prev.find(msg => msg.id === streamingMessageId);
            if (currentMessage && currentSession) {
              archieDB.addArchieMessage(currentSession.id, currentMessage).catch(dbError => {
      ...
      if (!isVisible) {
        archieBackgroundService.decrementBackgroundActivity();
      }
  ...
  const handleSelectSession = useCallback(async (sessionId: string) => {
    ...
    try {
      const session = await archieDB.getArchieSession(sessionId);
      if (session) {
  ...
  const handleSessionAction = useCallback(async (sessionId: string, action: 'star' | 'archive' | 'delete') => {
    try {
      if (action === 'delete') {
        ...
        await archieDB.deleteArchieSession(sessionId);
        ...
        if (currentSession?.id === sessionId) {
          ...
          if (remainingSessions.length > 0) {
            ...
            const nextSession = await archieDB.getArchieSession(remainingSessions[0].id);
            if (nextSession) {
        ...
        await archieDB.updateArchieSession(sessionId, updates);
        setSessions(prev => prev.map(s => 
  ...
  const handleProviderChange = useCallback(async (providerId: string) => {
    try {
      ...
      if (provider.type === 'archies-pocket' && window.llamaSwap) {
        try {
          console.log("🚀 Switching to Archie's Core - checking status...");
          // Check if running
          const status = await window.llamaSwap.getStatus?.();
          if (!status?.isRunning) {
            console.log("🔄 Archie's Core is not running, starting for provider switch...");
            addInfoNotification(
              "Starting Archie's Core...",
              'Archie is starting up her local AI service. Please wait a moment.',
              6000
            );
            const result = await window.llamaSwap.start();
            if (!result.success) {
              ...
              console.error("❌ Failed to start Archie's Core for provider switch:", result.error);
              setIsLoadingProviders(false);
              return;
            }
            console.log("✅ Archie's Core started successfully for provider switch");
            addInfoNotification(
              "Archie's Core Ready",
            ...
            console.log("✅ Archie's Core is already running for provider switch");
          }
        } catch (err) {
          console.error("⚠️ Error starting Archie's Core for provider switch:", err);
          addErrorNotification(
            "Archie's Core Startup Error",
            err instanceof Error ? err.message : 'Could not communicate with the local AI service.',
            8000
      ...
      archieApiService.updateProvider(provider);
      
      // STEP 3: Load models ONLY from the selected provider
      const newModels = await archieApiService.getModels(providerId);
  ...
  const handlePreloadModel = useCallback(async () => {
    ...
    if (sessionConfig.aiConfig.provider === 'archies-pocket') {
      try {
        const status = await window.llamaSwap?.getStatus();
        if (!status?.isRunning) {
          console.log('🚀 Starting local server...');
          await archieApiService.preloadModel(sessionConfig.aiConfig, messages);
        }
  ...
  const handleConfigChange = useCallback((newConfig: Partial<ArchieSessionConfig>) => {
    setSessionConfig(prev => {
  ...
  useEffect(() => {
    ...
    (window as any).debugArchieProviders = () => {
    ...
    (window as any).debugMCP = async () => {
      ...
      console.log('MCP Service Ready:', archieMCPService.isReady());
      console.log('Available Servers:', archieMCPService.getRunningServers());
      console.log('Available Tools:', archieMCPService.getAvailableTools());
    ...
    (window as any).setupTestMCP = async () => {
      ...
      const success = await archieMCPService.setupTestGitHubServer();
      if (success) {
        console.log('✅ Test MCP server setup complete');
        await archieMCPService.refresh();
        console.log('📊 Updated MCP status:', {
          servers: archieMCPService.getRunningServers().length,
          tools: archieMCPService.getAvailableTools().length
        });
      } else {
    ...
    (window as any).debugBackground = () => {
      console.log('🔄 Archie Background Service Status:');
      console.log(archieBackgroundService.getStatus());
    ...
    (window as any).testBackgroundChat = async () => {
      ...
      if (isVisible) {
        console.log('⚠️ Archie is currently visible. Switch to another page to test background mode.');
        return;
      }
      
      // Simulate a background message
      await handleSendMessage('This is a test message sent while Archie is in background mode.');
    };
    ...
    (window as any).testBackgroundService = () => {
      console.log('🧪 Testing Archie background service notification...');
      // Simulate Archie going to background mode
      archieBackgroundService.setBackgroundMode(true);
      ...
      setTimeout(() => {
        archieBackgroundService.setBackgroundMode(false);
        console.log('👁️ Returned to foreground');
      }, 5000);
    };

    // Add refresh functionality to debug utilities
    (window as any).refreshArchieServices = async (force = false) => {
      console.log('🔄 Manually refreshing Archie services...');
    ...
    (window as any).debugProblematicTools = (providerId?: string) => {
      ...
      if (providerId) {
        ...
        const storageKey = `archie-problematic-tools-${providerId}`;
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (key && key.startsWith('archie-problematic-tools-')) {
    ...
    (window as any).debugArchie = () => {
      console.log('Archie Status:', {
    ...
    (window as any).testCompleteAutonomousWorkflow = () => {
      ...
      const testMessage: ArchieMessage = {
        id: generateId(),
        role: 'user',
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          const resultMessage: ArchieMessage = {
            id: generateId(),
            role: 'assistant',
...
Console output:`;
    ...
    (window as any).debugVerificationSystem = () => {
      ...
      console.log('🔍 📊 Current Archie Configuration:');
      ...
      console.log('  - Current Provider:', archieApiService.getCurrentProvider()?.name || 'none');
      console.log('  - Current Client:', archieApiService.getCurrentClient() ? 'available' : 'none');
      console.log('');
    ...
    (window as any).debugMemory = () => {
      ...
      archieMemoryService.debugMemory();
    };
    ...
    (window as any).testMemory = () => {
      ...
      archieMemoryService.startSession('test-session', 'test-user');
      
      // Store some test tool results
      archieMemoryService.storeToolResult({
      ...
      const context = archieMemoryService.generateMemoryContext();
      console.log('Generated memory context:', context);
      
      // Show stats
      const stats = archieMemoryService.getMemoryStats();
      console.log('Memory stats:', stats);
      
      // Clear test session
      archieMemoryService.clearCurrentSession();
      console.log('✅ Memory test completed');
    };

    (window as any).clearMemory = () => {
      console.log('🧠 Clearing all memory...');
      archieMemoryService.clearAllMemory();
...
Let me execute this for you.`;
    ...
    (window as any).testNativeJSONSchemaComparison = async () => {
      ...
      const exampleRequest = {
        ...
        messages: [
          {
            role: "system",
            content: "You are Archie, an autonomous AI agent. Accomplish tasks using available tools."
          },
          {
            role: "user", 
    ...
    (window as any).testNativeJSONSchema = () => {
      ...
      const testTools = [
        {
          id: 'mcp_ArchieTools_py',
          name: 'mcp_ArchieTools_py',
        ...
        {
          id: 'mcp_ArchieTools_ls',
          name: 'mcp_ArchieTools_ls', 
    ...
    (window as any).testImageExtraction = () => {
      ...
      const { archieImageExtractionService } = require('../services/archieImageExtractionService');
      ...
      try {
        const extractedImages = archieImageExtractionService.extractImagesFromToolResults(
          mockToolResults,
          testMessageId
        );
        
        console.log('✅ Successfully extracted images:', extractedImages);
        console.log('📊 Storage stats:', archieImageExtractionService.getStorageStats());
        
        // Test retrieval
        if (extractedImages.length > 0) {
          const retrievedImage = archieImageExtractionService.getImage(extractedImages[0].id);
          console.log('✅ Successfully retrieved image:', retrievedImage?.id);
          
          const messageImages = archieImageExtractionService.getImagesForMessage(testMessageId);
    ...
    (window as any).testImageCleanup = () => {
      ...
      console.log('📊 Before cleanup:', archieImageExtractionService.getStorageStats());
      
      // Clean images older than 1 second (for testing)
      archieImageExtractionService.cleanupOldImages(1000);
      
      console.log('📊 After cleanup:', archieImageExtractionService.getStorageStats());
      ...
      const realToolResult = {
        ...
        result: {
          content: [
            {
              type: 'text',
              text: "Screenshot 'archievise.com_screenshot' taken at 800x600"
            },
            {
              type: 'image',
    ...
    return () => {
      delete (window as any).debugArchieProviders;
      ...
      delete (window as any).refreshArchieServices;
      ...
      delete (window as any).debugArchie;
...
${data.timezone ? `• **Timezone:** ${data.timezone}` : ''}`;
  ...
  return (
    <div className="flex h-screen w-full relative" data-archie-container>
      {/* Wallpaper */}
      {wallpaperUrl && (
        <div 
      ...
      <div className="relative z-10 flex h-screen w-full">
        ...
        <Sidebar activePage="archie" onPageChange={onPageChange} />
              ...
              <div className="max-w-4xl mx-auto transition-all duration-300">
                <AdvancedOptions
                  ...
                  onConfigChange={(newConfig) => {
                    ...
                    const updatedConfig: ArchieAIConfig = {
        ...
        {/* Archie Chat History Sidebar on the right */}
        <ArchieSidebar 
          sessions={sessions}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-8 m-4 max-w-md w-full mx-auto transform transition-all duration-300 ease-out scale-100 animate-in fade-in-0 zoom-in-95">
            ...
            <p className="text-gray-600 dark:text-gray-300 text-center mb-6 leading-relaxed">
              You don't seem to have any AI models downloaded yet. To start chatting with Archie, 
...
export default ArchieAssistant; 
