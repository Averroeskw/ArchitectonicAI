...
import { ArchieModel, ArchieProvider } from '../types/archie_assistant_types';
...
// Add TypeScript dearchietion for webview
dearchie global {
  namespace JSX {
...
const MODEL_CONFIGS_KEY = 'archie-ollama-model-configs';
...
const LAST_USED_LLM_KEY = 'archie-ollama-last-used-llm';
...
const ImageGen: React.FC<ImageGenProps> = ({ onPageChange }) => {
  ...
  const LAST_USED_MODEL_KEY = 'archie-ollama-last-used-model';
  const COMFYUI_CONSENT_KEY = 'archie-comfyui-user-consent';
  ...
  const [availableModels, setAvailableModels] = useState<ArchieModel[]>([]);
  ...
  const handleModelSelection = (model: string) => {
    ...
    if (savedConfig) {
      ...
      setNotificationMessage('Archie loaded optimal settings for this model');
    }
      ...
      switch (provider.type) {
        ...
        case 'openrouter':
          ...
          headers = {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${provider.apiKey}`,
            'HTTP-Referer': 'https://archieverse.app',
            'X-Title': 'ArchieVerse ImageGen Enhancement'
          };
          requestBody = {
            model: model.name,
            messages: messages,
        ...
        case 'archies-pocket':
          // Archie's Pocket - OpenAI-compatible API (base URL already includes /v1)
          apiUrl = `${provider.baseUrl || 'http://localhost:8080'}/chat/completions`;
          headers = {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${provider.apiKey || 'archie-key'}`
          };
          requestBody = {
            model: model.name,
            messages: messages,
  ...
  const handleEnhancePrompt = async (currentPrompt: string, imageData?: { preview: string; buffer: ArrayBuffer; base64: string }) => {
    ...
    try {
      ...
      // Use Archie's API service for the enhancement
  ...
  const fetchLLMModels = async (): Promise<ArchieModel[]> => {
    try {
      console.log('🔄 Fetching models from all configured providers...');
      
      // Get all enabled providers
      const allProviders = await archieApiService.getProviders();
    ...
    <div className="flex h-screen relative">
      ...
      <div className="relative z-10 flex h-screen w-full">
        ...
        <div className="flex-1 flex flex-col">
          ...
          {/* Conditional rendering: Show either Archie's interface or ComfyUI interface */}
        ...
        {/* Settings Drawer - Only show in Archie interface mode */}
        {!showComfyUIInterface && (
          <SettingsDrawer
            drawerRef={edgeRef}
            showSettings={showSettings}
