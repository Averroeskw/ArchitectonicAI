...
async function execute(inputs, properties, context) {
    ...
    const loadProviders = async () => {
      try {
        ...
        if (enabledProviders.length > 0) {
          ...
          const models = await archieApiService.getModels(primaryProvider.id);
  ...
  const handleProviderChange = async (providerId: string) => {
    ...
    try {
      const models = await archieApiService.getModels(providerId);
      setAvailableModels(models);
...
- Add relevant tags`
      ...
      // Use archieApiService to make the structured output request
      // First, ensure the provider is set correctly
      const currentProvider = archieApiService.getCurrentProvider();
      if (!currentProvider || currentProvider.id !== state.selectedProvider) {
        // Update the provider if needed
        archieApiService.updateProvider(selectedProviderData);
      }
      ...
      // Make the request using archieApiService's internal client
      const client = archieApiService['client'];
      if (!client) {
