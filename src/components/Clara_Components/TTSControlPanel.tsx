...
import { archieTTSService, AudioControlState, AudioProgress } from '../../services/archieTTSService';
...
const TTSControlPanel: React.FC<TTSControlPanelProps> = ({ 
  ...
  useEffect(() => {
    const unsubscribeState = archieTTSService.onStateChange((state) => {
      setAudioState(state);
    });

    const unsubscribeProgress = archieTTSService.onProgressUpdate((progress) => {
    ...
    const handleGlobalMouseUp = () => {
      if (isDraggingProgress) {
        ...
        archieTTSService.seekToProgress(tempProgress);
      }
    };
  ...
  const handlePlayPause = useCallback(() => {
    if (audioState?.isPlaying) {
      archieTTSService.pauseAudio();
    } else if (audioState?.isPaused) {
      archieTTSService.resumeAudio();
    }
  }, [audioState]);
  ...
  const handleVolumeChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isDraggingVolume) {
      archieTTSService.setVolume(volume);
    }
  }, [isDraggingVolume]);
  ...
  const handleVolumeDragEnd = useCallback(() => {
    ...
    archieTTSService.setVolume(tempVolume);
  }, [tempVolume]);
  ...
  const handleProgressChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    archieTTSService.seekToProgress(progress);
  ...
  const handleSpeedChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    archieTTSService.setSpeed(speed);
  }, []);

  // Handle volume mute toggle
  const handleVolumeToggle = useCallback(() => {
    if (audioState?.volume === 0) {
      archieTTSService.setVolume(0.7);
      setTempVolume(0.7);
    } else {
      archieTTSService.setVolume(0);
      setTempVolume(0);
    }
  }, [audioState?.volume]);
