 ...
 * 
 * Enhanced content renderer for Archie messages that supports:
...
interface MessageContentRendererProps {
  ...
  isDark?: boolean;
  isStreaming?: boolean;
  attachments?: ArchieFileAttachment[]; // Add attachments prop
}
...
const InlineMermaidRenderer: React.FC<{ content: string; isDark?: boolean }> = ({ content, isDark = false }) => {
  ...
  const isValidMermaid = (code: string): boolean => {
    ...
    // Must have at least one diagram type dearchietion
    const hasDearchietion = lines.some(line => 
    ...
    return hasDearchietion || hasConnections;
  };
  ...
  useEffect(() => {
    ...
    renderTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        // Ensure diagram has a proper dearchietion first
        if (!cleanCode.match(/^(graph|flowchart|sequenceDiagram|classDiagram|stateDiagram|erDiagram|journey|gitgraph|pie|quadrantChart|timeline|mindmap|sankey|c4Context|c4Container|c4Component|c4Dynamic|c4Deployment)/m)) {
          // Add a graph dearchietion if missing
          cleanCode = `graph TD\n${cleanCode}`;
        }
        ...
        try {
          // First try to render the original code (with dearchietion if needed)
          ...
          if (!isValidMermaid(fixedCode)) {
            throw new Error('Invalid Mermaid syntax - missing diagram dearchietion or connections');
          }
