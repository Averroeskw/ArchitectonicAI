/**
 * Archie Assistant Message Bubble Component
 * 
 * This component renders individual chat messages in the Archie assistant interface.
...
// Import TTS service
import { archieTTSService, AudioControlState } from '../../services/archieTTSService';
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[14px] font-medium ${getStatusColor()}`}>
            ...
            {getStatusText() === "thinking..." ? "Archie is  " : "Thought for "} {getStatusText()} {"herself"}
            </span>
          </span>
        </div>
        <div className="flex items-center gap-2">
...
const FileAttachmentDisplay: React.FC<{ 
  attachment: ArchieFileAttachment;
  onPreview?: (attachment: ArchieFileAttachment) => void;
...
/**
 * Main Archie Message Bubble Component
 */
const ArchieMessageBubble: React.FC<ArchieMessageBubbleProps> = ({
  message,
  userName = 'You',
  isEditable = false,
  onCopy,
  ...
  useEffect(() => {
    const unsubscribe = archieTTSService.onHealthChange((isHealthy) => {
  ...
  useEffect(() => {
    const unsubscribe = archieTTSService.onStateChange((state) => {
      setAudioState(state);
  ...
  const handleTTSToggle = async () => {
    ...
    try {
      ...
      await archieTTSService.synthesizeAndPlayWithControls({
        text: cleanContent,
        engine: 'kokoro',
        voice: 'af_sarah',
        speed: 1.0,
  ...
  const handleFilePreview = (attachment: ArchieFileAttachment) => {
...
export default ArchieMessageBubble; 