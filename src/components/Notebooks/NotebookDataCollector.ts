import { archieNotebookService } from '../../services/archieNotebookService';
...
class NotebookDataCollector {
  async collectNotebookData(notebookId: string): Promise<NotebookContent> {
    try {
      ...
      const notebook = await archieNotebookService.getNotebook(notebookId);
