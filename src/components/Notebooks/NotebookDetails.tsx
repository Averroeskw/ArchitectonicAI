...
import { 
  archieNotebookService, 
  NotebookResponse, 
  NotebookDocumentResponse 
} from '../../services/archieNotebookService';
import { useProviders } from '../../contexts/ProvidersContext';
import { archieApiService } from '../../services/archieApiService';
import { ArchieModel } from '../../types/archie_assistant_types';
...
const NotebookDetails: React.FC<NotebookDetailsProps> = ({ 
  ...
  const loadDocuments = async () => {
    if (!archieNotebookService.isBackendHealthy()) {
    ...
    try {
      const data = await archieNotebookService.listDocuments(notebook.id);
      setDocuments(data);
    } catch (err) {
  ...
  const loadModels = async () => {
    try {
      const allModels = await archieApiService.getModels();
      setModels(allModels);
    } catch (error) {
  ...
  const handleDocumentUpload = async (files: File[]) => {
    ...
    try {
      const uploadedDocs = await archieNotebookService.uploadDocuments(notebook.id, files);
  ...
  const handleDeleteDocument = async (documentId: string) => {
    ...
    try {
      await archieNotebookService.deleteDocument(notebook.id, documentId);
              ...
              <div className="glassmorphic-card p-6 rounded-lg">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className={`flex items-center gap-2 ${archieNotebookService.isBackendHealthy() ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      <div className={`w-2 h-2 rounded-full ${archieNotebookService.isBackendHealthy() ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      {archieNotebookService.isBackendHealthy() ? 'Connected' : 'Disconnected'}
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
              ...
              <div className="glassmorphic-card p-6 rounded-lg bg-red-50/50 dark:bg-red-900/10">
                ...
                <div className="space-y-3">
                  <button
                    ...
                    disabled={!archieNotebookService.isBackendHealthy()}
