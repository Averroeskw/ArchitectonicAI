...
import { archieNotebookService, NotebookResponse, ProviderConfig } from '../../services/archieNotebookService';
...
const NotebooksContent: React.FC<{ onPageChange: (page: string) => void; userName?: string }> = ({ onPageChange: _onPageChange, userName: _userName }) => {
  ...
  const loadNotebooks = async () => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      const data = await archieNotebookService.listNotebooks();
      setNotebooks(data);
    } catch (err) {
  ...
  const handleCreateNotebook = async (name: string, description: string, llmProvider: ProviderConfig, embeddingProvider: ProviderConfig) => {
    ...
    try {
      const newNotebook = await archieNotebookService.createNotebook({
        name,
  ...
  const handleDeleteNotebook = async (id: string) => {
    ...
    try {
      await archieNotebookService.deleteNotebook(id);
