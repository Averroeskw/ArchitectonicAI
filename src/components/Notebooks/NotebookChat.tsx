...
import { archieNotebookService, NotebookCitation } from '../../services/archieNotebookService';
...
const NotebookChat: React.FC<NotebookChatProps> = ({ notebookId, documentCount, completedDocumentCount, onDocumentUpload }) => {
  ...
  useEffect(() => {
    const unsubscribe = archieNotebookService.onHealthChange(setIsBackendHealthy);
    return unsubscribe;
  }, []);

  // Listen for progress updates from Archie assistant
  useEffect(() => {
    ...
    window.addEventListener('archie-progress-update', handleProgressUpdate as EventListener);

    // Also listen for console logs from Archie assistant and parse them
    const originalLog = console.log;
    console.log = (...args) => {
      originalLog.apply(console, args);
      
      // Check if this is a Archie progress log
    ...
    return () => {
      window.removeEventListener('archie-progress-update', handleProgressUpdate as EventListener);
  ...
  const generateDocumentSummary = async () => {
    ...
    try {
      ...
      // Start API request
      const summaryPromise = archieNotebookService.generateSummary(notebookId);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Query the notebook using the archieNotebookService
      const response = await archieNotebookService.queryNotebook(notebookId, {
