...
import { archieNotebookService, GraphData } from '../../services/archieNotebookService';
...
const GraphViewer: React.FC<GraphViewerProps> = ({ notebookId, onClose }) => {
  ...
  const loadHtmlGraph = async () => {
    ...
    try {
      const response = await fetch(archieNotebookService.getGraphHtmlUrl(notebookId));
      if (!response.ok) {
  ...
  const loadGraphData = async () => {
    ...
    try {
      const data = await archieNotebookService.getGraphData(notebookId);
      setGraphData(data);
    } catch (err) {
