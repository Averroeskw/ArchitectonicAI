...
const Sidebar = ({ activePage = 'dashboard', onPageChange, alphaFeaturesEnabled = false }: SidebarProps) => {
  ...
  const [archieBackgroundActivity, setArchieBackgroundActivity] = useState(false);
  ...
  // Listen for Archie background activity changes
  useEffect(() => {
    const handleArchieActivity = (event: CustomEvent) => {
      setArchieBackgroundActivity(event.detail.active);
    };

    window.addEventListener('archie-background-activity', handleArchieActivity as EventListener);
    return () => window.removeEventListener('archie-background-activity', handleArchieActivity as EventListener);
  }, []);
  ...
  return (
    ...
    >
      <div className={`flex items-center py-4 ${
        ...
        >
          <img src={logo} alt="Archie Logo" className="w-8 h-8 flex-shrink-0" />
          <h1 
            ...
            <li key={item.id}>
              ...
              >
                <div className="relative">
                  ...
                  {/* Background activity indicator for Archie */}
                  {item.id === 'archie' && archieBackgroundActivity && (
                ...
                >
                  ...
                  {item.id === 'archie' && archieBackgroundActivity && isExpanded && (
