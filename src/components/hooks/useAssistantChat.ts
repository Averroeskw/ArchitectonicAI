      ...
      if (selectedTool || useAllTools) {
        ...
        if (response.message?.tool_calls && response.message.tool_calls.length > 0) {
          ...
          for (const toolCall of response.message.tool_calls) {
            ...
            setMessages((prev) =>
              prev.map((msg) =>
                msg.id === assistantMessage.id ? { ...msg, content: `Archie is using tool - ${selectedTool.name}...` } : msg
              )
            );
          ...
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === assistantMessage.id ? { ...msg, content: `Archie is working on summarizing the results...` } : msg
            )
          );
