...
const Onboarding = ({onComplete}: OnboardingProps) => {
    ...
    const [formData, setFormData] = useState({
        ...
        avatar_url: '',
        archie_core_url: 'http://localhost:8091',
        comfyui_url: 'http://localhost:8188',
        model_folder_path: '',
        openai_api_key: '',
        openai_base_url: 'https://api.openai.com/v1',
        api_type: 'archie_core' as 'archie_core' | 'openai'
    });
    // const [loading, setLoading] = useState(false);
    const [archieStatus, setArchieStatus] = useState<'idle' | 'success' | 'error'>('idle');
    ...
    const [serviceModes, setServiceModes] = useState<{[key: string]: 'docker' | 'manual'}>({
        comfyui: 'docker',
        tts: 'docker', // Always docker for Archie's service
        n8n: 'docker'
    });
    ...
    // Use archieStatus to avoid lint warning
    console.log('Archie status:', archieStatus);
    ...
    // Auto-check Archie Core status when reaching step 4
    useEffect(() => {
        if (step === 4) {
            checkArchieCore();
        }
    ...
    const checkArchieCore = async () => {
        setArchieStatus('success'); // Archie Core is always available
    ...
    const handleSubmit = async () => {
        ...
        await db.updateAPIConfig({
            ollama_base_url: formData.archie_core_url,
        ...
        if ((window as any).featureConfig?.updateFeatureConfig) {
            try {
                await (window as any).featureConfig.updateFeatureConfig({
                    ...
                    archieCore: true, // Always enabled
            ...
            try {
                await (window as any).electronAPI.createUserConsentFile({
                    ...
                    services: {
                        ...
                        n8n: selectedServices.n8n,
                        'archie-core': true // Always enabled
                    },
    ...
    const features = [
        ...
        {
            title: "Powerful AI",
            description: "Access state-of-the-art AI models through Archie Core with built-in model management and optimization.",
                            ...
                            <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12">
                                <div className="w-full lg:w-1/2 text-center lg:text-left space-y-4 sm:space-y-6">
                                    ...
                                    <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 dark:text-white animate-fadeIn leading-tight">
                                        Welcome to <span className="text-sakura-500">Archie</span>
                                    </h1>
                        ...
                        {step === 1 ? "First, tell us a bit about yourself" :
                            step === 2 ? "How can we reach you?" :
                                step === 3 ? "Choose your preferred theme" :
                                    step === 4 ? "Let's connect to Archie Core" :
                ...
                <div className="space-y-6">
                    {step === 1 && (
                        <div className="space-y-4 animate-fadeIn">
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                Your name helps personalize your experience with Archie.
                            </p>
                            <input
                                type="text"
                                value={formData.name}
                    ...
                    {step === 4 && (
                        <div className="space-y-4 animate-fadeIn">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                                    Connect to Archie Core
                                </h3>
                            </div>
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                Archie Core is your local AI engine that processes everything privately on your device.
                            </p>

                            {/* Archie Core Connection Status */}
                            <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                                <div className="flex items-center gap-2 text-green-700 dark:text-green-300 mb-2">
                                    <Check className="w-5 h-5"/>
                                    <span className="font-medium">Archie Core is ready and running!</span>
                                </div>
                                <p className="text-sm text-green-600 dark:text-green-400">
                                    Connected to Archie Core at {formData.archie_core_url}
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 5 && (
                        <div className="space-y-4 animate-fadeIn">
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                Let's get your first AI model to start conversations with Archie. This will download a real model file (~350MB) to your computer.
                                ...
                                <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                                    ...
                                    {availableModels.length > 0 && (
                                        <p className="text-xs text-green-600 dark:text-green-400 mt-2">
                                            🎉 Your models are ready! You can start chatting with Archie right after setup.
                                        </p>
                                    )}
                                </div>
                            )}
                                    ...
                                    <div className="flex items-center gap-3 p-4">
                                        ...
                                        <div className="flex-1">
                                            <label htmlFor="tts-service" className="cursor-pointer">
                                                ...
                                                <p className="text-sm text-gray-600 dark:text-gray-400">Document analysis & voice synthesis (Archie's own service)</p>
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    {selectedServices.tts && (
                                        <div className="px-4 pb-4 space-y-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-gray-500">Archie's integrated service</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                                        Archie uses your timezone for time-aware responses.
                                    </p>
                                    <select
                                ...
                                <div className="p-3 bg-sakura-50 dark:bg-sakura-900/20 rounded-lg">
                                    ...
                                    <p className="text-sm text-sakura-700 dark:text-sakura-300">
                                        Archie is configured and ready to go. Click "Launch Archie" to start your AI-powered journey!
                                    </p>
                                </div>
                            </div>
                        </div>
