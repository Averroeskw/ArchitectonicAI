...
export const searchDocuments = async (
  ...
  try {
    ...
    // If RAG is enabled and no temp docs, use only archie-assistant collection
    if (ragEnabled) {
      try {
        const response = await fetch(`http://0.0.0.0:${pythonPort}/documents/search`, {
          method: 'POST',
          ...
          body: JSON.stringify({
            query,
            collection_name: 'archie-assistant',
            k: 8,
          }),
        });
        if (response.ok) {
          ...
          console.warn('Archie Assistant collection search failed:', response.status);
          return { results: [] };
        }
      } catch (error) {
        console.warn('Archie Assistant collection search error:', error);
        return { results: [] };
      }
    }
