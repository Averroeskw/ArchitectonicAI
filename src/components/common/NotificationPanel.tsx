...
import { notificationService, ArchieNotification } from '../../services/notificationService';

interface NotificationPanelProps {
  className?: string;
  onNavigateToArchie?: () => void;
}

const NotificationPanel: React.FC<NotificationPanelProps> = ({ className = '', onNavigateToArchie }) => {
  const [notifications, setNotifications] = useState<ArchieNotification[]>([]);
  ...
  const handleBackgroundNotificationClick = (notification: ArchieNotification) => {
    // If it's a background Archie notification, navigate to Archie and mark as read
    if (notification.title.includes('Archie Response Ready') && onNavigateToArchie) {
      onNavigateToArchie();
      handleMarkAsRead(notification.id);
      setIsOpen(false);
    }
    // If it's the background service notification, navigate to Archie but don't mark as read
    else if (notification.title.includes('Archie Assistant Active') && onNavigateToArchie) {
      onNavigateToArchie();
      setIsOpen(false);
    }
  };

  const getNotificationIcon = (type: ArchieNotification['type']) => {
    switch (type) {
      case 'completion':
  ...
  const getNotificationColor = (type: ArchieNotification['type']) => {
    switch (type) {
      case 'completion':
  ...
  const isBackgroundServiceNotification = (notification: ArchieNotification) => {
    return notification.title.includes('Archie Assistant Active');
  };

  const isArchieResponseNotification = (notification: ArchieNotification) => {
    return notification.title.includes('Archie Response Ready');
  };
          ...
          <div className="max-h-80 overflow-y-auto">
            {notifications.length === 0 ? (
              ...
              notifications.map((notification) => {
                ...
                const isArchieResponse = isArchieResponseNotification(notification);
                ...
                return (
                  <div
                    ...
                    title={
                      isArchieResponse 
                        ? 'Click to go to Archie chat' 
                        : isBackgroundService 
                        ? 'Archie is running in background - click to go to Archie'
                        : undefined
                    }
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          {isBackgroundService && (
                            <span className="text-xs text-green-600 dark:text-green-400 font-medium" title="Click to go to Archie">
                              Background Service
                            </span>
                          )}
                          {isArchieResponse && (
                            <span className="text-xs text-sakura-600 dark:text-sakura-400 font-medium" title="Click to go to Archie">
                              → Archie
                            </span>
                          )}
                        </div>
