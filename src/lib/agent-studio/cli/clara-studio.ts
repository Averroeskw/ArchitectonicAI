...
/**
 * Archie Agent Studio CLI
 * 
 * Command-line interface for the Archie Agent Studio framework.
...
program
  .name('archie-studio')
  .description('Archie Agent Studio - Visual AI Workflow Builder')
  .version(version);
...
program
  .command('init')
  .description('Initialize a new Archie Studio project')
...
async function generateNodeCommand(description: string | undefined, options: any) {
  ...
  try {
    ...
    console.log(chalk.gray('  3. Start development: archie-studio dev'));
...
async function startDevMode(options: any) {
  console.log(chalk.cyan('🚀 Starting Archie Studio development mode...\n'));
...
async function initProjectCommand(name: string | undefined, options: any) {
  const projectName = name || 'my-archie-studio-project';
  ...
  try {
    ...
    console.log(chalk.gray('  archie-studio dev'));
