...
import { archieMemoryService } from './archieMemoryService';
...
import { archieImageExtractionService } from './archieImageExtractionService';
...
export class ArchieAgentService {
  ...
  private async executeAutonomousAgentWithStructuredCalling(
    ...
    archieMemoryService.startSession(sessionId, 'structured-autonomous-agent');
    ...
    if (autonomousConfig.enableProgressTracking && onContentChunk) {
      onContentChunk('**Archie is now operating in structured autonomous mode.**\n\n');
    ...
    try {
      ...
      for (let iteration = 0; iteration < autonomousConfig.maxIterations; iteration++) {
        ...
        try {
          ...
          if (structuredResult.needsToolExecution && structuredResult.toolCalls.length > 0) {
            ...
            const conversationWithResults: ArchieMessage[] = [];
            ...
            // Convert Archie messages to ChatMessage format for API call
            const chatMessages: ChatMessage[] = [];
            
            // Add system prompt
            chatMessages.push({
              role: 'system',
              content: systemPrompt || 'You are Archie, a helpful AI assistant.'
            });
      ...
      // Create final Archie message
      const archieMessage: ArchieMessage = {
      ...
      if (allToolResults.length > 0) {
        archieMessage.artifacts = archieToolService.parseToolResultsToArtifacts(allToolResults);
        ...
        try {
          ...
          if (extractedImages.length > 0) {
            ...
            if (!archieMessage.metadata) {
              archieMessage.metadata = {};
            }
            archieMessage.metadata.extractedImages = extractedImages;
  ...
  private async executeAutonomousAgentWithStandardToolCalling(
    ...
    archieMemoryService.startSession(sessionId, 'autonomous-agent');
    ...
    // Progress tracking
    if (autonomousConfig.enableProgressTracking && onContentChunk) {
      onContentChunk('**Archie is now operating in autonomous mode.**\n\n');
    }
  ...
  private async executeAgentIteration(
    ...
    try {
      ...
      if (apiResponse.message?.tool_calls && apiResponse.message.tool_calls.length > 0) {
        ...
        // Execute tools
        toolResults = await archieToolService.executeToolCalls(toolCalls, currentProviderId);
  ...
  private buildAutonomousSystemPrompt(
    ...
    return `You are Archie, an autonomous AI agent. Your core directive: ACCOMPLISH THE TASK using available tools.
...
Work autonomously and resourcefully to fulfill the user's request.`;
  ...
  private createToolMasteryPrompt(tools: Tool[]): string {
    ...
    return `You are Archie, an autonomous problem-solving agent. Your mission: SOLVE ANY TASK using the tools at your disposal.
...
export const archieAgentService = new ArchieAgentService(); 