...
import { defaultTools, executeTool } from '../utils/archieTools';
import { db } from '../db';
import type { Tool } from '../db';
import { archieMCPService } from './archieMCPService';
import { ToolSuccessRegistry } from './toolSuccessRegistry';
import type { ArchieAIConfig, ArchieArtifact, ArchieMCPToolResult } from '../types/archie_assistant_types';
import type { ChatMessage } from '../utils/APIClient';

export class ArchieToolService {
  /**
   * Get available tools based on configuration
   */
  public async getAvailableTools(config: ArchieAIConfig, onContentChunk?: (content: string) => void): Promise<Tool[]> {
    let tools: Tool[] = [];
        ...
        try {
          if (archieMCPService.isReady()) {
            ...
            if (enabledServers.length === 0) {
              ...
              const serverSummary = archieMCPService.getServerAvailabilitySummary(enabledServers);
              ...
              const mcpTools = archieMCPService.getToolsFromEnabledServers(enabledServers);
              ...
              if (mcpTools.length === 0) {
                ...
                const mcpOpenAITools = archieMCPService.convertSpecificToolsToOpenAIFormat(mcpTools);
  ...
  /**
   * Execute tool calls using the Archie tools system
   */
  public async executeToolCalls(toolCalls: any[], currentProviderId?: string): Promise<any[]> {
    ...
    for (const toolCall of toolCalls) {
      try {
        ...
        // Try to execute with Archie tools first
        const archieTool = defaultTools.find(tool => tool.name === functionName || tool.id === functionName);
        
        if (archieTool) {
          const result = await executeTool(archieTool.id, args);
          
          if (result.success) {
            ToolSuccessRegistry.recordSuccess(
              archieTool.name,
              archieTool.description,
  ...
  private async executeMCPToolCall(toolCall: any, functionName: string, currentProviderId?: string): Promise<any> {
    ...
    try {
      const mcpToolCalls = archieMCPService.parseOpenAIToolCalls([toolCall]);
      
      if (mcpToolCalls.length > 0) {
        const mcpResult = await archieMCPService.executeToolCall(mcpToolCalls[0]);
   ...
   */
  private processMCPToolResult(mcpResult: ArchieMCPToolResult, toolName: string): {
    result: any;
    artifacts: ArchieArtifact[];
    images: string[];
    toolMessage: ChatMessage;
  } {
    const artifacts: ArchieArtifact[] = [];
    const images: string[] = [];
    let textContent = '';
   ...
   */
  public parseToolResultsToArtifacts(toolResults: any[]): ArchieArtifact[] {
    const artifacts: ArchieArtifact[] = [];
...
export const archieToolService = new ArchieToolService(); 