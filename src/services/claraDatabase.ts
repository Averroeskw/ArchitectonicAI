...
export interface ArchieChatSessionRecord extends Omit<ArchieChatSession, 'createdAt' | 'updatedAt'> {
  createdAt: string; // Store as ISO string for IndexedDB
  updatedAt: string;
}

export interface ArchieMessageRecord extends Omit<ArchieMessage, 'timestamp'> {
  sessionId: string; // Reference to the session
  timestamp: string; // Store as ISO string for IndexedDB
}

export interface ArchieFileRecord {
  id: string;
  sessionId: string;
  messageId: string;
  name: string;
...
/**
 * Database service specifically for Archie chat sessions
 * Handles persistence of chat sessions, messages, and file attachments
 */
export class ArchieDatabaseService {
  private readonly SESSIONS_STORE = 'archie_sessions';
  private readonly MESSAGES_STORE = 'archie_messages';
  private readonly FILES_STORE = 'archie_files';

  /**
   * Save a chat session to the database
   */
  async saveSession(session: ArchieChatSession): Promise<void> {
    const sessionRecord: ArchieChatSessionRecord = {
      ...session,
   ...
   */
  async saveMessage(sessionId: string, message: ArchieMessage): Promise<void> {
    const messageRecord: ArchieMessageRecord = {
      ...message,
      sessionId,
   ...
   */
  async saveFile(sessionId: string, messageId: string, file: ArchieFileAttachment): Promise<void> {
    const fileRecord: ArchieFileRecord = {
      id: file.id,
      sessionId,
      messageId,
      name: file.name,
   ...
   */
  async getAllSessions(includeMessages: boolean = false): Promise<ArchieChatSession[]> {
    const sessionRecords = await indexedDBService.getAll<ArchieChatSessionRecord>(this.SESSIONS_STORE);
    
    // Convert back to ArchieChatSession objects and sort by updatedAt
   ...
   */
  async getSession(sessionId: string): Promise<ArchieChatSession | null> {
    const sessionRecord = await indexedDBService.get<ArchieChatSessionRecord>(this.SESSIONS_STORE, sessionId);
   ...
   */
  async getSessionMessages(sessionId: string): Promise<ArchieMessage[]> {
    const allMessages = await indexedDBService.getAll<ArchieMessageRecord>(this.MESSAGES_STORE);
    const sessionMessages = allMessages
      .filter(msg => msg.sessionId === sessionId)
      .sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());

    // Convert to ArchieMessage objects and load attachments
    const messages: ArchieMessage[] = [];
   ...
   */
  async getMessageFiles(messageId: string): Promise<ArchieFileAttachment[]> {
    const allFiles = await indexedDBService.getAll<ArchieFileRecord>(this.FILES_STORE);
   ...
   */
  async updateSession(sessionId: string, updates: Partial<ArchieChatSession>): Promise<void> {
    const existingRecord = await indexedDBService.get<ArchieChatSessionRecord>(this.SESSIONS_STORE, sessionId);
    if (!existingRecord) throw new Error(`Session ${sessionId} not found`);

    const updatedRecord: ArchieChatSessionRecord = {
      ...existingRecord,
      ...updates,
  ...
  async deleteSession(sessionId: string): Promise<void> {
    ...
    const messages = await indexedDBService.getAll<ArchieMessageRecord>(this.MESSAGES_STORE);
    ...
    const files = await indexedDBService.getAll<ArchieFileRecord>(this.FILES_STORE);
   ...
   */
  async getRecentSessions(limit: number = 20): Promise<ArchieChatSession[]> {
    const sessionRecords = await indexedDBService.getAll<ArchieChatSessionRecord>(this.SESSIONS_STORE);
   ...
   */
  async getRecentSessionsLight(limit: number = 20, offset: number = 0): Promise<ArchieChatSession[]> {
    const sessionRecords = await indexedDBService.getAll<ArchieChatSessionRecord>(this.SESSIONS_STORE);
    const allMessages = await indexedDBService.getAll<ArchieMessageRecord>(this.MESSAGES_STORE);
   ...
   */
  async getStarredSessions(): Promise<ArchieChatSession[]> {
   ...
   */
  async getArchivedSessions(): Promise<ArchieChatSession[]> {
    const sessions = await this.getAllSessions(true); // Include messages
   ...
   */
  async searchSessions(query: string): Promise<ArchieChatSession[]> {
  ...
  async getStorageStats(): Promise<{
    ...
    const sessions = await indexedDBService.getAll<ArchieChatSessionRecord>(this.SESSIONS_STORE);
    const messages = await indexedDBService.getAll<ArchieMessageRecord>(this.MESSAGES_STORE);
    const files = await indexedDBService.getAll<ArchieFileRecord>(this.FILES_STORE);
  ...
  async clearAllSessions(): Promise<void> {
    try {
      ...
      console.log('Successfully cleared all Archie chat data');
    } catch (error) {
      console.error('Failed to clear Archie chat data:', error);
      throw error;
    }
  }
  ...
  private recordToSession(record: ArchieChatSessionRecord): ArchieChatSession {
    return {
      ...record,
  ...
  private async recordToMessage(record: ArchieMessageRecord): Promise<ArchieMessage> {
  ...
  private recordToFileAttachment(record: ArchieFileRecord): ArchieFileAttachment {
...
export const archieDatabaseService = new ArchieDatabaseService(); 