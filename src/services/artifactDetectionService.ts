...
import { ArchieArtifact, ArchieArtifactType, ArchieAIConfig } from '../types/archie_assistant_types';
...
export interface DetectionContext {
  ...
  attachments?: any[];
  artifactConfig?: ArchieAIConfig['artifacts'];
}
...
export class ArtifactDetectionService {
   ...
   */
  private static extractCodeBlocks(content: string): { artifacts: ArchieArtifact[]; cleanedContent: string; confidence: number } {
    const artifacts: ArchieArtifact[] = [];
   ...
   */
  private static extractTables(content: string): { artifacts: ArchieArtifact[]; cleanedContent: string; confidence: number } {
    const artifacts: ArchieArtifact[] = [];
    let cleanedContent = content; // Keep original content instead of cleaning
    let confidence = 0;
   ...
   */
  private static extractMermaidDiagrams(content: string): { artifacts: ArchieArtifact[]; cleanedContent: string; confidence: number } {
    const artifacts: ArchieArtifact[] = [];
   ...
   */
  private static extractStructuredData(content: string): { artifacts: ArchieArtifact[]; cleanedContent: string; confidence: number } {
    const artifacts: ArchieArtifact[] = [];
    let cleanedContent = content;
    let confidence = 0;
   ...
   */
  private static extractMathFormulas(content: string): { artifacts: ArchieArtifact[]; cleanedContent: string; confidence: number } {
    const artifacts: ArchieArtifact[] = [];
    let cleanedContent = content;
    let confidence = 0;
   ...
   */
  private static detectFromContext(content: string, userMessage: string, conversationHistory: string[]): { artifacts: ArchieArtifact[]; cleanedContent: string; confidence: number } {
    const artifacts: ArchieArtifact[] = [];
    let cleanedContent = content;
    let confidence = 0;
   ...
   */
  private static detectJsonType(obj: any): ArchieArtifactType {
   ...
   * Detect array type
   */
  private static detectArrayType(arr: any[]): ArchieArtifactType {
   ...
   */
  private static isInlineVisualContentFromArtifact(artifact: ArchieArtifact): boolean {
