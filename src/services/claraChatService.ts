...
export class ArchieChatService {
  /**
  ...
  public async executeStandardChat(
    ...
    const messages = this.buildConversationMessages(
      systemPrompt || 'You are Archie, a helpful AI assistant.',
      message,
      attachments,
      conversationHistory
    );
    ...
    try {
      ...
      if (response.message?.tool_calls && response.message.tool_calls.length > 0) {
        ...
        toolResults = await archieToolService.executeToolCalls(response.message.tool_calls, currentProviderId);
    ...
    if (toolResults.length > 0) {
      archieMessage.artifacts = archieToolService.parseToolResultsToArtifacts(toolResults);
  ...
  public async preloadModel(
    ...
    try {
      const warmupMessages = [
        { role: 'system' as const, content: 'You are Archie, a helpful AI assistant.' },
...
export const archieChatService = new ArchieChatService(); 