/**
 * Archie Image Extraction Service
 * 
...
import { ArchieExtractedImage, ArchieMCPToolResult } from '../types/archie_assistant_types';

export class ArchieImageExtractionService {
  private static instance: ArchieImageExtractionService;
  private imageStorage: Map<string, ArchieExtractedImage> = new Map();

  public static getInstance(): ArchieImageExtractionService {
    if (!ArchieImageExtractionService.instance) {
      ArchieImageExtractionService.instance = new ArchieImageExtractionService();
    }
    return ArchieImageExtractionService.instance;
  }
  ...
  public extractImagesFromToolResults(
    toolResults: any[],
    messageId: string
  ): ArchieExtractedImage[] {
    const extractedImages: ArchieExtractedImage[] = [];
  ...
  private processImageContent(
    ...
    try {
      ...
      const storagePath = `archie-images/${imageId}`;
      ...
      const extractedImage: ArchieExtractedImage = {
        id: imageId,
        toolName,
        contentIndex,
        data: imageData,
  ...
  private extractBase64Images(
    ...
    const images: ArchieExtractedImage[] = [];
   ...
   * Update stored image
   */
  private updateStoredImage(imageId: string, updatedImage: ArchieExtractedImage): void {
    this.imageStorage.set(imageId, updatedImage);
    
    try {
      const storageKey = `archie-image-${imageId}`;
      const imageData = {
        ...updatedImage,
   ...
   */
  public getImage(imageId: string): ArchieExtractedImage | null {
    // Try memory first
   ...
   */
  public getImagesForMessage(messageId: string): ArchieExtractedImage[] {
    const images: ArchieExtractedImage[] = [];
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith('archie-image-')) {
...
export const archieImageExtractionService = ArchieImageExtractionService.getInstance(); 