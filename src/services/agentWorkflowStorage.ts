...
export interface ExportFormat {
  format: 'archie-native' | 'json' | 'n8n-compatible';
  data: any;
  metadata: {
    exportedAt: string;
    exportedBy: string;
...
export class AgentWorkflowStorage {
   ...
   */
  async exportWorkflow(id: string, format: ExportFormat['format'] = 'archie-native'): Promise<ExportFormat | null> {
    try {
      ...
      const exportFormat: ExportFormat = {
        ...
        metadata: {
          ...
          exportedBy: 'Archie Agent Builder',
          platform: 'archie',
  ...
  private detectFormat(data: any): string {
    ...
    // Check for new archie-sdk format (with nested flow structure)
    if (data.format === 'archie-sdk' && data.flow) return 'archie-sdk';
    
    // Check for archie-native format (with nested flow structure)
    if (data.format === 'archie-native' && data.flow) return 'archie-native-nested';
    
    // Check for direct flow structure (old format)
    if (data.nodes && data.connections && data.id) return 'archie-native';
  ...
  /**
   * Convert Archie workflow to n8n format (basic conversion)
   */
  private convertToN8NFormat(workflow: AgentFlow): any {
    ...
    return {
      ...
      nodes: workflow.nodes.map(node => ({
        name: node.name,
        type: this.mapArchieTypeToN8N(node.type),
  ...
  /**
   * Convert n8n workflow to Archie format (basic conversion)
   */
  ...
  /**
   * Map Archie node types to n8n types
   */
  private mapArchieTypeToN8N(archieType: string): string {
    ...
    return typeMap[archieType] || 'Code';
  }

  /**
   * Map n8n node types to Archie types
   */
  ...
  /**
   * Convert Archie connections to n8n format
   */
  ...
  /**
   * Convert n8n connections to Archie format
   */
