  ...
  private async initDB(): Promise<IDBDatabase> {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        // Add Archie-specific stores
        if (!db.objectStoreNames.contains('archie_sessions')) {
          const sessionStore = db.createObjectStore('archie_sessions', { keyPath: 'id' });
        ...
        if (!db.objectStoreNames.contains('archie_messages')) {
          const messageStore = db.createObjectStore('archie_messages', { keyPath: 'id' });
        ...
        if (!db.objectStoreNames.contains('archie_files')) {
          const fileStore = db.createObjectStore('archie_files', { keyPath: 'id' });
