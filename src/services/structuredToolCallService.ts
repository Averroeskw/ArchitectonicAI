...
import { ArchieMessage, ArchieAIConfig, ArchieFileAttachment } from '../types/archie_assistant_types';
import { Tool } from '../db';
...
export class StructuredToolCallService {
  ...
  private buildNativeStructuredPrompt(tools: Tool[], originalSystemPrompt?: string): string {
    ...
    return `You are Archie, an autonomous AI agent. Your task is to accomplish user requests using available tools.
...
Be thorough, accurate, and helpful in your responses.`;
  ...
  public generateStructuredToolPrompt(
    ...
    let systemPrompt = `You are Archie, an autonomous AI agent. ACCOMPLISH THE TASK using structured tool calls.
  ...
  private async executeStructuredTool(toolCall: StructuredToolCall): Promise<StructuredToolResult> {
    ...
    if (toolName.startsWith('mcp_')) {
      try {
        ...
        const mcpResult = await archieMCPService.executeToolCall(mcpToolCall);
    ...
    // Check Archie default tools
    const archieTool = defaultTools.find(tool => tool.name === toolName || tool.id === toolName);
    if (archieTool) {
      try {
        console.log(`🛠️ Executing Archie tool: ${archieTool.name}`);
        const result = await executeTool(archieTool.id, args);
        console.log(`✅ Archie tool result:`, result);
        ...
        console.error(`❌ Archie tool execution error for ${toolName}:`, error);
        return {
          toolName,
          success: false,
          error: error instanceof Error ? error.message : 'Archie tool execution failed',
          reasoning
        };
      }
    }
...
If any tools failed, acknowledge the failures and provide alternative suggestions or partial results where possible.`;
   ...
   */
  private formatConversationHistory(history?: ArchieMessage[]): string {
